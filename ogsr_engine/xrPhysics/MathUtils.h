...
IC void dVectorDeviation(const float* vector3_from, const float* vector3_to, float* vector_dev) { dVectorSub(vector_dev, vector3_to, vector3_from); }
...
IC bool check_obb_sise(const Fobb& obb) { return (!fis_zero(obb.m_halfsize.x, EPS_L) || !fis_zero(obb.m_halfsize.y, EPS_L) || !fis_zero(obb.m_halfsize.z, EPS_L)); }
