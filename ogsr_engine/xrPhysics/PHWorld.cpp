...
void  create_physics_world(CObjectSpace* os, CObjectList* lo, CRenderDeviceBase* dv) // IPHWorldUpdateCallbck &commander,
{
...
void  set_mtl_lib(CGameMtlLibrary* l) { PGMLib = l; }
...
CPHWorld::CPHWorld()
    : // IPHWorldUpdateCallbck		*_update_callback
      m_update_callback(&empty_update_callback), m_default_contact_shotmark(0), m_default_character_contact_shotmark(0), physics_step_time_callback(0), m_object_space(0),
      m_level_objects(0), m_device(0)
{
    disable_count = 0;
    m_frame_time = 0.f;
...
void CPHWorld::Destroy()
{
    ...
    ::Device.seqFrame.Remove(this); //SIMP: ???? сюда вроде не добавляем нигде, но пусть будет
