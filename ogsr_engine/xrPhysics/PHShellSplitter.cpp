...
void CPHShellSplitterHolder::SplitElement(u16 aspl, PHSHELL_PAIR_VECTOR& out_shels)
{
    ...
    for (; i != e; ++i)
    {
        ...
        VERIFY(out_shels.back().first->get_ElementByStoreOrder(0));
...
void CPHShellSplitterHolder::AddSplitter(CPHShellSplitter::EType type, u16 element, u16 joint) { m_splitters.push_back(CPHShellSplitter(type, element, joint)); }
