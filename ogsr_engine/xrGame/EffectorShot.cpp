...
{
    m_angle_vert += angle * (m_cam_recoil.DispersionFrac + Random.randF(-1.0f, 1.0f) * (1.0f - m_cam_recoil.DispersionFrac));

    clamp(m_angle_vert, -m_cam_recoil.MaxAngleVert, m_cam_recoil.MaxAngleVert);
    if (fis_zero(m_angle_vert - m_cam_recoil.MaxAngleVert))
    {
        m_angle_vert *= Random.randF(0.96f, 1.04f);
    }

    float rdm = Random.randF(-1.0f, 1.0f);
