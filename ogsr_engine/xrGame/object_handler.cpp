...
bool CObjectHandler::weapon_strapped(CWeapon* weapon) const
{
    ...
    {
        //		Msg						( "[%6d][%s] weapon_strapped = %s1", Device.dwTimeGlobal, planner().object().cName().c_str(), almost_strapped &&
...
bool CObjectHandler::weapon_unstrapped() const
{
    ...
    if (!weapon)
    {
        //		Msg						( "[%6d][%s] no active item!!(%d)(%s)", Device.dwTimeGlobal, planner().object().cName().c_str(), inventory().GetActiveSlot(),
        // inventory().ItemFromSlot(3) ? inventory().ItemFromSlot(3)->object().cName().c_str() : "<no_item>" );
        return (true);
    }
...
bool CObjectHandler::weapon_unstrapped(CWeapon* weapon) const
{
    ...
    if (almost_unstrapped || (planner().current_action_state_id() == ObjectHandlerSpace::eWorldOperatorStrapping2Idle) ||
        (planner().current_action_state_id() == ObjectHandlerSpace::eWorldOperatorStrapping) ||
        (planner().current_action_state_id() == ObjectHandlerSpace::eWorldOperatorUnstrapping))
    {
        //		Msg						( "[%6d][%s] weapon_unstrapped = %s1", Device.dwTimeGlobal, planner().object().cName().c_str(), almost_unstrapped &&
        //! planner().m_storage.property(ObjectHandlerSpace::eWorldPropertyStrapped2Idle) ? "true" : "false" );
