...
inline u32 CBucketList::compute_bucket_id(Vertex& vertex) const
{
    ...
    if (dist <= m_min_bucket_value)
        return 0;
    u32 idx = u32(BucketCount * (dist - m_min_bucket_value) / (m_max_bucket_value - m_min_bucket_value));
...
inline void CBucketList::add_to_bucket(Vertex& vertex, u32 m_bucket_id)
{
    ...
    ASSERT_FMT(m_bucket_id < BucketCount, "[CBucketList::add_to_bucket]: m_bucket_id < BucketCount: %u, %u", m_bucket_id, BucketCount);
...
TEMPLATE_SPECIALIZATION
inline void CBucketList::set_min_bucket_value(const Distance min_bucket_value) { m_min_bucket_value = min_bucket_value; }

TEMPLATE_SPECIALIZATION
inline void CBucketList::set_max_bucket_value(const Distance max_bucket_value) { m_max_bucket_value = max_bucket_value; }
