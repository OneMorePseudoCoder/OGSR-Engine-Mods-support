...
void CDamageManager::load_section(LPCSTR section, CInifile const* ini)
{
    string32 buffer;
    IKinematics* kinematics = smart_cast<IKinematics*>(m_object->Visual());
    const auto& damages = ini->r_section(section);
    for (const auto& [key, val] : damages.Ordered_Data)
    {
        if (xr_strcmp(key, "default"))
        { // read all except default line
            VERIFY(m_object);
            int bone = kinematics->LL_BoneID(key);
            R_ASSERT2(BI_NONE != bone, key.c_str());
            CBoneInstance& bone_instance = kinematics->LL_GetBoneInstance(u16(bone));
            bone_instance.set_param(0, (float)atof(_GetItem(val.c_str(), 0, buffer)));
            bone_instance.set_param(1, (float)atoi(_GetItem(val.c_str(), 1, buffer)));
            bone_instance.set_param(2, (float)atof(_GetItem(val.c_str(), 2, buffer)));
            if (_GetItemCount(val.c_str()) < 4)
            {
                bone_instance.set_param(3, (float)atof(_GetItem(val.c_str(), 0, buffer)));
            }
            else
            {
                bone_instance.set_param(3, (float)atof(_GetItem(val.c_str(), 3, buffer)));
            }
