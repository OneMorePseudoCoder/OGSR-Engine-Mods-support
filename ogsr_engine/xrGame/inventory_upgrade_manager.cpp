...
void Manager::load_all_properties()
{
    ...
    const auto& inv_section = pSettings->r_section(properties_section);
    for (const auto& property_id : inv_section.Ordered_Data | std::views::keys)
    {
...
bool Manager::make_known_upgrade(CInventoryItem& item, shared_str const& upgrade_id) { return (upgrade_verify(item.m_section_id, upgrade_id)->make_known()); }
...
bool Manager::is_known_upgrade(CInventoryItem& item, shared_str const& upgrade_id) { return (upgrade_verify(item.m_section_id, upgrade_id)->is_known()); }
...
void Manager::init_install(CInventoryItem& item)
{
    ...
    if (pSettings->line_exist(item.m_section_id, "installed_upgrades"))
    {
        ...
        if (installed_upgrades_str)
        {
            ...
            {
                upgrade_install(item, __GetItem(installed_upgrades_str, i, temp, buffer_size), true);
            }
        }
    } // if exist
}
