...
Irect CLevelFogOfWar::ConvertRealToLocal(const Frect& src) { return Irect().set(ConvertRealToLocal(src.lt), ConvertRealToLocal(src.rb)); }

Fvector2 CLevelFogOfWar::ConvertLocalToReal(const Ivector2& src) { return Fvector2().set(m_levelRect.lt.x + src.x * FOG_CELL_SZ, m_levelRect.lt.y + src.y * FOG_CELL_SZ); }
