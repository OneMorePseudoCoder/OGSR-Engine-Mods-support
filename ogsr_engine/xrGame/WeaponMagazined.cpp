...
void CWeaponMagazined::Load(LPCSTR section)
{
    ...
    m_bUse2DShells = READ_IF_EXISTS(pSettings, r_bool, section, "use_2d_shells", true);
...
bool CWeaponMagazined::CanAttach(PIItem pIItem)
{
                ...
                (m_scopes[cur_scope]->m_sScopeName == pIItem->object().cNameSect())*/)
    {
        ...
        {
            if (shared_str(pSettings->r_string((*it), "scope_name")) == pIItem->object().cNameSect())
                return true;
        }
        return false;
    }
...
bool CWeaponMagazined::GetBriefInfo(II_BriefInfo& info)
{
    ...
    else
    {
        ...
        info.bag_ammo._set(int_str);

        xr_sprintf(int_str, "%d/%d", ae, total);
        info.full_ammo._set(int_str);
    }
