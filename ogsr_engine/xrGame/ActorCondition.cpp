...
BOOL GodMode() { return psActorFlags.test(AF_GODMODE | AF_GODMODE_RT); }
...
void CActorCondition::UpdateThirst()
{
    if (!fis_zero(m_fV_Thirst))
    {
        ...
        if (!psActorFlags.test(AF_GODMODE_RT))
        {
            m_fDeltaHealth += CanBeHarmed() ? (m_fV_ThirstHealth * (m_fThirst > 0.0f ? 1.f : -1.f) * m_fDeltaTime) : 0;
...
void CActorCondition::BoostAlcoholRestore(const float value) { m_fV_Alcohol += value; }
void CActorCondition::BoostSatietyRestore(const float value) { m_fV_Satiety += value; }
void CActorCondition::BoostThirstRestore(const float value) { m_fV_Thirst += value; }
...
bool CActorCondition::DisableSprint(SHit* pHDS)
{
    return (pHDS->hit_type != ALife::eHitTypeTelepatic) && (pHDS->hit_type != ALife::eHitTypeChemicalBurn) && (pHDS->hit_type != ALife::eHitTypeBurn) &&
        (pHDS->hit_type != ALife::eHitTypeLightBurn) && (pHDS->hit_type != ALife::eHitTypeRadiation);
}
    ...
    : m_pParent(parent)
{
    ...
    pe->m_on_b_remove_callback = fastdelegate::MakeDelegate(this, &CActorDeathEffector::OnPPEffectorReleased);
    m_b_actual = true;
