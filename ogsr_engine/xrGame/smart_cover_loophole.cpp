...
public:
    ...
    IC bool operator()(std::pair<shared_str, action*> const& other) const { return (m_id._get() == other.first._get()); }
};
...
void loophole::fill_transitions(luabind::object const& transitions_table)
{
    ...
    for (; I != E; ++I)
    {
        ...
        if (!m_transitions.vertex(action_from))
            m_transitions.add_vertex(std::monostate(), action_from);

        if (!m_transitions.vertex(action_to))
            m_transitions.add_vertex(std::monostate(), action_to);
