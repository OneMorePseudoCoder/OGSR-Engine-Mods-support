...
void CPurchaseList::process(CInifile& ini_file, LPCSTR section, CInventoryOwner& owner)
{
    ...
    const auto& S = ini_file.r_section(section);
    for (const auto& [key, val] : S.Ordered_Data)
    {
        VERIFY3(val.size(), "PurchaseList : cannot handle lines in section without values", section);

        string256 temp0, temp1;
        THROW3(_GetItemCount(val.c_str()) == 2, "Invalid parameters in section", section);
        process(game_object, key, atoi(_GetItem(val.c_str(), 0, temp0)), (float)atof(_GetItem(val.c_str(), 1, temp1)));
    }
}
