...
Flags32 g_mt_config = {mtLevelPath | mtDetailPath | mtObjectHandler | mtSoundPlayer | mtAiVision | mtBullets | mtLevelSounds | mtALife | mtMap};
#ifdef DEBUG
...
public:
    ...
    virtual void fill_tips(vecTips& tips, u32 mode)
    {
        ...
        for (const auto& section : pSettings->sections_ordered() | std::views::keys)
        {
...
public:
    ...
    virtual void Execute(LPCSTR args)
    {
        ...
        if (count > 250)
        {
            Msg("! [g_spawn_to_inventory]: Cancel the command. Maximum value of the second argument: 250. Count is: %d", count);
            return;
        }
...
void CCC_RegisterCommands()
{
    ...
    CMD4(CCC_Float, "hud_fov", &psHUD_FOV_def, 0.1f, 0.99f);
...
#pragma todo("SIMP: В будущем оставить команды для разрабов под ключиком наверное, пока для удобства оставлю включенными всегда")
    //if (Core.ParamFlags.test(Core.ParamFlag::dbg))
    {
    ...
    CMD4(CCC_Integer, "cod_pickup_mode", &g_b_COD_PickUpMode, 0, 1);

    CMD3(CCC_Mask, "g_autopickup", &psActorFlags, AF_AUTOPICKUP);
    CMD3(CCC_Mask, "g_dynamic_music", &psActorFlags, AF_DYNAMIC_MUSIC);
    CMD3(CCC_Mask, "g_important_save", &psActorFlags, AF_IMPORTANT_SAVE);

    psActorFlags.set(AF_MUSIC_TRACKS, TRUE); // Пусть по дефолту будет включено для ЗП.
    ...
    CMD2(CCC_Bool, "string_table_error_msg", &CStringTable::m_bWriteErrorsToLog);
...
#pragma todo("SIMP: см. комментарии в CEntityAlive::get_new_local_point_on_mesh")
