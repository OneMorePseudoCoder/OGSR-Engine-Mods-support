...
shared_str parse_vertex(luabind::object const& table, LPCSTR identifier, bool const& in) { return (transform_vertex(parse_string(table, identifier), in)); }
} // namespace smart_cover
...
void description::load_loopholes(shared_str const& table_id)
{
    ...
    if(!result)
        Msg("!!bad or missing loopholes table in smart_cover [%s]", table_id.c_str());
...
void description::load_transitions(shared_str const& table_id)
{
    ...
    if (!result)
        Msg("!!bad or missing transitions table in smart_cover [%s]", table_id.c_str());
    ...
    for (; I != E; ++I)
    {
        ...
        if (!m_transitions.vertex(vertex_0_id))
            m_transitions.add_vertex(std::monostate(), vertex_0_id);

        if (!m_transitions.vertex(vertex_1_id))
            m_transitions.add_vertex(std::monostate(), vertex_1_id);
...
loophole const* description::loophole(shared_str const& loophole_id) const
{
    ...
    public:
        ...
        IC bool operator()(smart_cover::loophole const* loophole) const { return (m_id._get() == loophole->id()._get()); }
    };
