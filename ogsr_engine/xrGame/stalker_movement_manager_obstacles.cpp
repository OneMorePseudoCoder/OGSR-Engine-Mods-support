...
bool stalker_movement_manager_obstacles::can_build_restricted_path(const obstacles_query& query)
{
    ...
    m_failed_to_build_path = !ai().graph_engine().search(ai().level_graph(), object().ai_location().level_vertex_id(), level_path().dest_vertex_id(), &m_temp_path,
...
const float& stalker_movement_manager_obstacles::prediction_speed() const { return (object().animation().target_speed()); }
