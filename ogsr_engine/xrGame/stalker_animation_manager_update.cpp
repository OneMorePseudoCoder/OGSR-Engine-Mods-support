...
void CStalkerAnimationManager::update()
{
    ...
    {
        Msg("! error in stalker [%s], profile [%s] with visual [%s]", object().cNameSect().c_str(),
            object().cast_inventory_owner()->CharacterInfo().GetSpecificCharacterId().c_str(), object().cNameVisual().c_str());
