...
cover::cover(smart_cover::object const& object, DescriptionPtr description, bool const is_combat_cover, bool const can_fire, luabind::object const& loopholes_availability)
    : inherited(object.Position(), object.ai_location().level_vertex_id()), m_object(object), m_description(description), m_id(m_object.cName()),
      m_is_combat_cover(is_combat_cover), m_can_fire(can_fire)
{
    m_is_smart_cover = 1;
    ...
    for (; J != K; ++J, ++i)
    {
        ...
        position.y += 2.0f;
        VERIFY2(graph.valid_vertex_id(graph.vertex_id(position)), make_string("invalid vertex id: smart cover[%s], loophole [%s]", m_object.cName().c_str(), (*I)->id().c_str()));
        vertex(**J, (*i).second);
...
void cover::vertex(smart_cover::loophole const& loophole, smart_cover::loophole_data& loophole_data)
{
    ...
    VERIFY2(graph.valid_vertex_id(loophole_data.m_level_vertex_id),
            make_string("invalid vertex id: smart cover[%s], loophole [%s]", m_object.cName().c_str(), loophole.id().c_str()));
