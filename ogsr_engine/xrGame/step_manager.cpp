...
void CStepManager::reload(LPCSTR section)
{
    ...
    if (debug_step_info_load)
        Msg("loading step_params for object :[%s], visual: [%s], section: [%s], step_params section: [%s]", m_object->cName().c_str(), m_object->cNameVisual().c_str(), section,
            anim_section);
#endif
    ...
    for (u32 i = 0; pSettings->r_line(anim_section, i, &anim_name, &val); ++i)
    {
        ...
        ASSERT_FMT(param.cycles >= 1, "!![%s] invalid cycles count [%u] for object: [%s], visual: [%s], section: [%s], step_params section: [%s]", __FUNCTION__, param.cycles,
        ...
        if (!motion_id)
        {
            ...
            Msg("! (CStepManager::reload) no anim :%s object:%s, visual: %s, step_params section: %s ", anim_name, m_object->cName().c_str(), m_object->cNameVisual().c_str(),
                anim_section);
        ...
        if (debug_step_info_load)
        {
            ...
            Msg("step_params loaded for object :%s, visual: %s, motion: %s, anim set: %s  ", m_object->cName().c_str(), m_object->cNameVisual().c_str(), anim_name.first,
                anim_name.second);
        }
#endif
...
void CStepManager::on_animation_start(MotionID motion_id, CBlend* blend)
{
    ...
    if (it == m_steps_map.end())
    {
        ...
        if (debug_step_info)
        {
            ...
            VERIFY(KA);
            std::pair<LPCSTR, LPCSTR> anim_name = KA->LL_MotionDefName_dbg(motion_id);
            Msg("! no step_params found for object :%s, visual: %s, motion: %s, anim set: %s  ", m_object->cName().c_str(), m_object->cNameVisual().c_str(), anim_name.first,
                anim_name.second);
        }
#endif
...
{
    for (const auto& [key, val] : data.Ordered_Data)
    {
        u16 index = smart_cast<IKinematics*>(m_object->Visual())->LL_BoneID(val);
        VERIFY3(index != BI_NONE, "foot bone not found", val.c_str());

        if (xr_strcmp(key, "front_left") == 0)
            m_foot_bones[eFrontLeft] = index;
        else if (xr_strcmp(key, "front_right") == 0)
            m_foot_bones[eFrontRight] = index;
        else if (xr_strcmp(key, "back_right") == 0)
            m_foot_bones[eBackRight] = index;
        else if (xr_strcmp(key, "back_left") == 0)
