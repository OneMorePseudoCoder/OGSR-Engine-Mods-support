...
action_base* animation_selector::current_operator() const { return (&smart_cast<smart_cover::action_base&>(m_planner->current_action())); }
...
MotionID animation_selector::select_animation(bool& animation_movement_controller)
{
    ...
    if (m_callback_called)
    {
		...
		LPSTR				result;
		xr_strconcat			( result, m_animation, "_slot_", animation_slot_string);

		MotionID			animation_id = m_skeleton_animated->ID_Cycle_Safe( result );
		if (animation_id)
			return			(animation_id);

		xr_strconcat			( result, m_animation, "_slot_2" );
