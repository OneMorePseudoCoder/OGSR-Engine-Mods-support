...
void CGrenade::Load(LPCSTR section)
{
    ...
    m_bExplosiveWhileNotActivated = READ_IF_EXISTS(pSettings, r_bool, section, "explosive_while_not_activated", true);

    if (m_bExplosionOnHit)
    {
        const char* types = pSettings->r_string(section, "explosion_hit_types");
        string128 item;

        for (int i = 0, count = _GetItemCount(types); i < count; ++i)
        {
            _GetItem(types, i, item);
            m_uExplosionHitTypes.push_back(atoi(item));
        }
    }
...
bool CGrenade::ExplodeOnHit(SHit* pHDS)
{
    if (!m_bExplosionOnHit || m_grenade_detonation_threshold_hit >= pHDS->power)
        return false;

    if (Useful() && !m_bExplosiveWhileNotActivated)
        return false;

    if (m_uExplosionHitTypes.empty())
        return pHDS->hit_type == ALife::eHitTypeExplosion;

    for (const auto& hitType : m_uExplosionHitTypes)
    {
        if (pHDS->hit_type == static_cast<ALife::EHitType>(hitType))
            return true;
    }
...
{
    if (ALife::eHitTypeExplosion == pHDS->hit_type && m_grenade_detonation_threshold_hit < pHDS->damage() && CExplosive::Initiator() == u16(-1) || ExplodeOnHit(pHDS))
    {
...
bool CGrenade::NeedToDestroyObject() const { return false; }
