...
bool CScriptEntity::bfAssignMovement(CScriptEntityAction* tpEntityAction)
{
    ...
    case CScriptMovementAction::eGoalTypePathPosition: {
        ...
        {
            string256 S;
            xr_sprintf(S, "Cannot find corresponding level vertex for the specified position [%f][%f][%f] for monster %s", VPUSH(l_tMovementAction.m_tDestinationPosition),
                       *m_monster->cName());
...
bool CScriptEntity::bfScriptAnimation()
{
    ...
    if (GetScriptControl() && GetCurrentAction() && !GetCurrentAction()->m_tAnimationAction.m_bCompleted && xr_strlen(GetCurrentAction()->m_tAnimationAction.m_caAnimationToPlay))
    {
        ...
        for (u16 i = 0; i < MAX_PARTS; ++i)
        {
            ...
            VERIFY(skeleton_animated->LL_GetMotionDef(animation));
