...
void CPHCommander::update()
{
    ...
    {
#pragma todo("FIX THIS! в main ветке должны быть фиксы.")
        try
        {
            m_calls[i]->check();
        }
...
void CPHCommander::add_call(CPHCondition* condition, CPHAction* action) { m_calls.push_back(xr_new<CPHCall>(condition, action)); }
...
void CPHCommander::remove_calls(CPHReqComparerV* cmp_object) { m_calls.erase(std::remove_if(m_calls.begin(), m_calls.end(), SRemoveRped(cmp_object)), m_calls.end()); }
