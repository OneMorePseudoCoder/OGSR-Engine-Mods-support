...
void CPHDestroyable::Load(CInifile* ini, LPCSTR section)
{
    ...
    {
        const auto& data = ini->r_section(section);
        if (!data.Ordered_Data.empty())
        {
            m_flags.set(fl_destroyable, TRUE);
            for (const auto& name : data.Ordered_Data | std::views::keys)
                if (name.size())
                    m_destroyed_obj_visual_names.push_back(name);
        }
    }
}
