...
//имя секции таблицы
TEMPLATE_SPECIALIZATION
const char* CSIni_Table::table_sect = nullptr;
TEMPLATE_SPECIALIZATION
...
typename CSIni_Table::ITEM_TABLE& CSIni_Table::table()
{
    ...
    string64 buffer;
    const auto& table_ini = pSettings->r_section(table_sect);

    R_ASSERT3(table_ini.Ordered_Data.size() == table_size, "wrong size for table in section", table_sect);

    for (const auto& [key, val] : table_ini.Ordered_Data)
    {
        auto cur_index = T_INI_LOADER::IdToIndex(key, std::numeric_limits<typename T_INI_LOADER::index_type>::max());

        if (std::numeric_limits<typename T_INI_LOADER::index_type>::max() == cur_index)
            Debug.fatal(DEBUG_INFO, "wrong community %s in section [%s]", key.c_str(), table_sect);

        (*m_pTable)[cur_index].resize(cur_table_width);
        for (std::size_t j = 0; j < cur_table_width; j++)
        {
            (*m_pTable)[cur_index][j] = convert(_GetItem(val.c_str(), (int)j, buffer));
        }
    }
