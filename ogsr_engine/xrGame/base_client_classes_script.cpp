...
void CObjectScript::script_register(lua_State* L)
{
    module(L)[
        class_<CGameObject, bases<DLL_Pure, ISheduled, ICollidable, IRenderable>, CGameObjectWrapper>("CGameObject")
            ...
            //SIMP: сомневаюсь что в скриптах это для чего то может быть нужно
...
void IKinematicsAnimatedScript::script_register(lua_State* L) { module(L)[class_<IKinematicsAnimated>("IKinematicsAnimated").def("PlayCycle", &IKinematicsAnimated_PlayCycle)]; }
