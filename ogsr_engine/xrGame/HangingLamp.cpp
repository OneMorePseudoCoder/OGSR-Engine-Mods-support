...
BOOL CHangingLamp::net_Spawn(CSE_Abstract* DC)
{
    ...
    if (need_force_volumetric)
    {
        ...
        // Для спота ставим такие значения, потому что в спавне спотов волюметрик вообще мог не быть включен и там слишком задранные значения, которые будут плохо выглядеть с новым волюметриком
    ...
    else
    {
        ...
        //Для поинта будем надеяться что в спавне подходящие значения под наш волюметрик, это надо будет проверить. Если вдруг что, поставить те же значения как для спотов выше
        //Пока просто ограничил их по нижнему пределу. Надеюсь, они не будут нигде сильно задраны.
        light_render->set_volumetric_intensity(std::max(0.25f, lamp->m_volumetric_intensity));
        light_render->set_volumetric_distance(std::max(1.f, lamp->m_volumetric_distance));
    }
