...
void CALifeSwitchManager::remove_online(CSE_ALifeDynamicObject* object, bool update_registries)
{
    ...
    if (auto* inventory_owner = smart_cast<CSE_ALifeTraderAbstract*>(object))
    {
        m_saved_chidren.erase(std::remove_if(m_saved_chidren.begin(), m_saved_chidren.end(),
                                             [this](const ALife::_OBJECT_ID& id) {
                                                 if (auto* _object = server().game->get_entity_from_eid(id))
                                                     if (auto* alife_object = smart_cast<CSE_ALifeObject*>(_object))
