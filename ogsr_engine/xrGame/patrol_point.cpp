...
const GameGraph::_GRAPH_ID& CPatrolPoint::game_vertex_id() const
{
    ...
    VERIFY2(vertex,
            make_string("invalid game vertex id[%d] (level_vertex_id[%d]) for patrol point[%s] in path[%s] in position[%f][%f][%f]", m_game_vertex_id, m_level_vertex_id,
                        m_name.c_str(), m_path->m_name.c_str(), VPUSH(m_position)));
