...
float stalker_movement_manager_smart_cover::enter_path(LoopholePath* result, Fvector const& position, u32 const level_vertex_id, smart_cover::cover const& cover,
...
stalker_movement_manager_smart_cover::transition_action const& stalker_movement_manager_smart_cover::nearest_action(smart_cover::cover const& cover, shared_str const& loophole_id0,
                                                                                                                    ...
                                                                                                                    EBodyState* target_body_state) const
{
    ...
    VERIFY2(result,
            make_string("cover[%s][%s], loophole[%s -> %s], body_state[%s] [%f][%f][%f]", cover.id().c_str(), cover.description()->table_id().c_str(), loophole_id0.c_str(),
                        loophole_id1.c_str(),
...
stalker_movement_manager_smart_cover::transition_action const& stalker_movement_manager_smart_cover::action(smart_cover::cover const& cover, shared_str const& loophole_id0,
...
void stalker_movement_manager_smart_cover::build_exit_path_to_cover()
{
    ...
    for (; I != E; ++I)
    {
        ...
        smart_cover::transitions::action const& current_action =
            nearest_action(current_cover, exitable_loophole_id, smart_cover::transform_vertex("", false), target_position, exit_position, exit_vertex_id, &exit_body_state);
...
static IC bool exit_loophole(shared_str const& loophole_id) { return (loophole_id._get() == smart_cover::transform_vertex("", false)._get()); }
...
float const& stalker_movement_manager_smart_cover::idle_min_time() const { return (animation_selector().planner().idle_min_time()); }

void stalker_movement_manager_smart_cover::idle_min_time(float const& value) { animation_selector().planner().idle_min_time(value); }

float const& stalker_movement_manager_smart_cover::idle_max_time() const { return (animation_selector().planner().idle_max_time()); }

void stalker_movement_manager_smart_cover::idle_max_time(float const& value) { animation_selector().planner().idle_max_time(value); }

float const& stalker_movement_manager_smart_cover::lookout_min_time() const { return (animation_selector().planner().lookout_min_time()); }

void stalker_movement_manager_smart_cover::lookout_min_time(float const& value) { animation_selector().planner().lookout_min_time(value); }

float const& stalker_movement_manager_smart_cover::lookout_max_time() const { return (animation_selector().planner().lookout_max_time()); }

void stalker_movement_manager_smart_cover::lookout_max_time(float const& value) { animation_selector().planner().lookout_max_time(value); }
