...
Flags32 psActorFlags = {AF_GODMODE_RT | AF_AUTOPICKUP | AF_RUN_BACKWARD | AF_IMPORTANT_SAVE | AF_DOF_RELOAD};
...
CActor::CActor() : CEntityAlive()
{
    ...
    pPickup = xr_new<CPickUpManager>(this);
...
void CActor::Hit(SHit* pHDS)
{
    ...
    if ((mstate_real & mcSprint) && Level().CurrentControlEntity() == this && conditions().DisableSprint(pHDS))
    {
        bool const is_special_burn_hit_2_self =
            (pHDS->who == this) && (pHDS->boneID == BI_NONE) && ((pHDS->hit_type == ALife::eHitTypeBurn) || (pHDS->hit_type == ALife::eHitTypeLightBurn));
...
void CActor::HitSignal(float perc, Fvector& vLocalDir, CObject* who, s16 element)
{
    if (g_Alive())
    {
        ...
        auto* tpKinematics = smart_cast<IKinematicsAnimated*>(pV);
        auto* pK = smart_cast<IKinematics*>(pV);
        VERIFY(tpKinematics);
...
void CActor::Die(CObject* who)
{
    ...
    for (; I <= E; ++I)
    {
        ...
        {
            auto* pOutfit = smart_cast<CCustomOutfit*>(item_in_slot);
            if (pOutfit)
                continue;
        };
        if (item_in_slot)
...
void CActor::UpdateCL()
{
    ...
    if (CWeapon* pWeapon = smart_cast<CWeapon*>(inventory().ActiveItem()))
    {
        if (pWeapon->IsZoomed())
        {
            ...
            auto* S = smart_cast<CEffectorZoomInertion*>(Cameras().GetCamEffector(eCEZoom));
            if (S)
...
#pragma todo("SIMP: подумать над тем что передавать в y и w. Это надо для 3DSS в будущем.")
...
void CActor::shedule_Update(u32 DT)
{
    ...
    */
    ...
    const bool can_use = true; //! active_hud || active_hud->GetState() == CHudItem::eIdle || !Core.Features.test(xrCore::Feature::busy_actor_restrictions);

    if (can_use && !input_external_handler_installed() && !m_holder && RQ.O && RQ.O->getVisible() && RQ.range < 2.0f)
    {
...
bool CActor::NeedToDestroyObject() const { return false; }
...
void CActor::OnItemDrop(CInventoryItem* inventory_item, bool just_before_destroy)
{
    ...
    if (!just_before_destroy && inventory_item->BaseSlot() == GRENADE_SLOT && nullptr == inventory().ItemFromSlot(GRENADE_SLOT))
    {
...
void CActor::OnItemDropUpdate()
{
    ...
    {
        if (item && !item->IsInvalid() && !attached(item))
            attach(item);
    }
}

void CActor::OnItemRuck(CInventoryItem* inventory_item, const SInvItemPlace& previous_place) { CInventoryOwner::OnItemRuck(inventory_item, previous_place); }

void CActor::OnItemBelt(CInventoryItem* inventory_item, const SInvItemPlace& previous_place) { CInventoryOwner::OnItemBelt(inventory_item, previous_place); }

#define ARTEFACTS_UPDATE_TIME 0.100f

void CActor::UpdateArtefactsOnBeltAndOutfit()
{
    // Pilot: вместо переменных update_time, f_update_time сделаем одну статическую
    static float accumulated_time = 0.0f;

    // апдейтим таймер
    accumulated_time += conditions().fdelta_time();
    if (accumulated_time < ARTEFACTS_UPDATE_TIME)
        return;

    const float dt = accumulated_time;
    ...
    for (auto* item : inventory().m_belt)
    {
        ...
        cond.ChangeBleeding(artefact->m_fBleedingRestoreSpeed * dt);
        cond.ChangeHealth(artefact->m_fHealthRestoreSpeed * dt);
        cond.ChangePower(artefact->m_fPowerRestoreSpeed * dt);
        cond.ChangeSatiety(artefact->m_fSatietyRestoreSpeed * dt);
        cond.ChangeThirst(artefact->m_fThirstRestoreSpeed * dt);

        const float base_rad = artefact->m_fRadiationRestoreSpeed;
        if (base_rad > 0.0f)
        {
            float val = base_rad - cond.GetBoostRadiationImmunity();
            val = std::max(val, 0.0f);
            cond.ChangeRadiation(val * dt);
        }
        else
        {
            cond.ChangeRadiation(base_rad * dt);
        }
    }
    ...
    {
        auto& cond = conditions();
        cond.ChangeBleeding(outfit->m_fBleedingRestoreSpeed * dt);
        cond.ChangeHealth(outfit->m_fHealthRestoreSpeed * dt);
        cond.ChangePower(outfit->m_fPowerRestoreSpeed * dt);
        cond.ChangeSatiety(outfit->m_fSatietyRestoreSpeed * dt);
        cond.ChangeThirst(outfit->m_fThirstRestoreSpeed * dt);
        cond.ChangeRadiation(outfit->m_fRadiationRestoreSpeed * dt);
        return; // есть костюм — выходим
    }

    // Нет костюма — проверяем шлем и ПНВ
    if (CHelmet* helmet = smart_cast<CHelmet*>(inventory().ItemFromSlot(HELMET_SLOT)))
        return;
...
float CActor::HitArtefactsOnBelt(float hit_power, ALife::EHitType hit_type)
{
    ...
    return std::clamp(hit_power, 0.0f, flt_max);
}

float CActor::GetProtection_ArtefactsOnBelt(ALife::EHitType hit_type)
{
    float protection_sum = 0.0f;

    for (auto* item : inventory().m_belt)
    {
        if (auto* artefact = smart_cast<CArtefact*>(item))
            protection_sum += artefact->m_ArtefactHitImmunities.AffectHit(1.0f, hit_type);
    }

    return protection_sum;
}

void CActor::SetZoomRndSeed(s32 seed) { m_ZoomRndSeed = (seed != 0) ? seed : s32(Level().timeServer_Async()); }

void CActor::SetShotRndSeed(s32 seed) { m_ShotRndSeed = (seed != 0) ? seed : s32(Level().timeServer_Async()); }
...
void CActor::PickupModeOn() { pPickup->SetPickupMode(true); }

void CActor::PickupModeOff() { pPickup->SetPickupMode(false); }
