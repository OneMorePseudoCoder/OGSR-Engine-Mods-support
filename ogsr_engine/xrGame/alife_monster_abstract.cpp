...
void CSE_ALifeMonsterAbstract::vfCheckForPopulationChanges()
{
    ...
    if (l_tTimeID >= l_tpALifeGroupAbstract->m_tNextBirthTime)
    {
        ...
        if (Random.randF(100) < ai().ef_storage().m_pfBirthProbability->ffGetValue())
        {
            u32 l_dwBornCount = iFloor(float(l_tpALifeGroupAbstract->m_wCount) * Random.randF(.5f, 1.5f) * ai().ef_storage().m_pfBirthPercentage->ffGetValue() / 100.f + .5f);
            if (l_dwBornCount)
            {
...
bool CSE_ALifeMonsterAbstract::redundant() const
{
    ...
    VERIFY2(m_game_death_time <= current_time,
            make_string("incorrect death time for monster %s[death time = %I64d][current time = %I64d]", name_replace(), m_game_death_time, current_time));
