...
CInventory::CInventory()
{
    ...
    const u32 slotsCount = pSettings->r_u32("inventory", "slots_count");
    ...
    m_last_slot = i;
#ifndef MASTER_GOLD
    if (m_last_slot != slotsCount)
    {
        Log("~ Not critical, but check [inventory] section in your system.ltx.");
        Msg("~ slots_count = %u, but real slots count is %u", slotsCount, m_last_slot);
    }
#endif
...
bool CInventory::Slot(u16 slot_id, PIItem pIItem, bool bNotActivate, bool strict_placement)
{
    ...
    {
#ifdef DEBUG
        Msg("there is item %s[%d,%x] in slot %d[%d,%x]", ItemFromSlot(pIItem->GetSlot())->object().cName().c_str(), ItemFromSlot(pIItem->GetSlot())->object().ID(),
            ItemFromSlot(pIItem->GetSlot()), pIItem->GetSlot(), pIItem->object().ID(), pIItem);
#endif
...
void CInventory::Update()
{
    ...
    {
        auto* pActor_owner = smart_cast<CObject*>(m_pOwner);
        if (Level().CurrentViewEntity() == pActor_owner)
        {
...
PIItem CInventory::Same(const PIItem pIItem, bool bSearchRuck) const
{
    ...
    for (CInventoryItem *const l_pIItem : list)
    {
