...
void CEncyclopediaArticle::InitXmlIdToIndex()
{
    ...
    {
        static const bool enable_encyclopedia = READ_IF_EXISTS(pSettings, r_bool, "features", "enable_encyclopedia", true);
        if (!enable_encyclopedia)
            id_to_index::file_str = "";
        else
            id_to_index::file_str = READ_IF_EXISTS(pSettings, r_string, "encyclopedia", "files", "");
    }
}
