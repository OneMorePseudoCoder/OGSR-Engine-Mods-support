...
void CUISequenceSimpleItem::Update()
{
    ...
    //Simp: не понятно зачем это надо, я альттабнулся из игры во время загрузки, а она мне на апдейте курсор дергает когда загрузка закончилась
...
void CUISequenceSimpleItem::OnKeyboardPress(int dik)
{
    ...
    for (u32 idx = 0; idx < m_actions.size(); ++idx)
    {
        ...
        if (b)
        {
            ...
            ASSERT_FMT_DBG(functor_exists, "!!Cannot find script function described in tutorial item [%s]", itm.m_functor.c_str());
            functor_to_call();
