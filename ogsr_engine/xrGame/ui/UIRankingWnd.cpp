...
void CUIRankingWnd::Show(bool status)
{
    if (status)
    {
        ...
        m_money_value->SetText(std::format("{} {}", Actor()->get_money(), StMoneyDescr).c_str());
...
void CUIRankingWnd::Init()
{
    ...
    for (const auto& arch : achievs_section.Ordered_Data | std::views::keys)
        add_achievement(xml, arch);
