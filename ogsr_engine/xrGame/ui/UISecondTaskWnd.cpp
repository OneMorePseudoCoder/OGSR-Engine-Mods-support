...
void UITaskListWnd::init_from_xml(CUIXml& xml, LPCSTR path)
{
    ...
    Register(m_bt_close);
    AddCallback(m_bt_close, BUTTON_DOWN, fastdelegate::MakeDelegate(this, &UITaskListWnd::OnBtnClose));
...
{
    if (static_cast<int>(iDirection) == WINDOW_MOUSE_WHEEL_UP)
        m_list->ScrollBar()->TryScrollDec();
    else if (static_cast<int>(iDirection) == WINDOW_MOUSE_WHEEL_DOWN)
