...
void CUIMapWnd::Init(LPCSTR xml_name, LPCSTR start_from)
{
    ...
    if (m_scroll_mode)
    {
        ...
        AddCallback(m_UIMainScrollV, SCROLLBAR_VSCROLL, fastdelegate::MakeDelegate(this, &CUIMapWnd::OnScrollV));
    }
    ...
    {
        const auto& S = pGameIni->r_section(sect_name.c_str());
        for (const auto& _map_name : S.Ordered_Data | std::views::keys)
        {
            shared_str map_name = _map_name;
            xr_strlwr(map_name);
