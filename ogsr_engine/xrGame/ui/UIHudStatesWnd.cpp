...
void CUIHudStatesWnd::InitFromXml(CUIXml& xml, LPCSTR path)
{
    ...
    */
    ...
    m_ui_weapon_bag_ammo = UIHelper::CreateStatic(xml, "static_bag_ammo", this, false);
    ...
    if (m_ui_weapon_icon = UIHelper::CreateStatic(xml, "static_wpn_icon", this, false))
    {
    ...
    m_arrow->init_from_xml(xml, "arrow", this, false);
    m_arrow_shadow->init_from_xml(xml, "arrow_shadow", this, false);
...
void CUIHudStatesWnd::UpdateActiveItemInfo(CActor* actor)
{
    ...
    if (item)
    {
        ...
        if (m_ui_weapon_sign_ammo)
        {
            ...
            m_ui_weapon_sign_ammo->SetText(m_item_info.full_ammo.c_str());
            /* if (UI().is_widescreen())
            {
                pFont = pFont22;
            }
            ...
            {
                if (m_item_info.full_ammo.size() > 5)
                {
                    pFont = pFont22;
                }
                else
        ...
        if (m_ui_weapon_bag_ammo)
        {
            m_ui_weapon_bag_ammo->Show(true);
            m_ui_weapon_bag_ammo->SetText(m_item_info.bag_ammo.c_str());
        }
        if (m_fire_mode)
        {
    ...
    else
    {
        ...
        if (m_ui_weapon_bag_ammo)
            m_ui_weapon_bag_ammo->Show(false);
        if (m_ui_weapon_sign_ammo)
...
void CUIHudStatesWnd::UpdateZones()
{
    ...
    if (pda)
    {
        ...
        {
            CBaseMonster* const monster = smart_cast<CBaseMonster*>(O);
