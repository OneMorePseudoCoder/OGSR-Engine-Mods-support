...
CUIWindow::CUIWindow()
    : m_pParentWnd(nullptr), m_pMouseCapturer(nullptr), m_pMessageTarget(nullptr), m_pKeyboardCapturer(nullptr), m_bAutoDelete(false), m_bCursorOverWindow(false), m_bPP(false),
      m_dwFocusReceiveTime(0), m_bCustomDraw(false)
{
    Show(true);
    Enable(true);
#ifdef LOG_ALL_WNDS
...
bool CUIWindow::OnMouseAction(float x, float y, EUIMessages mouse_action)
{
    ...
    WINDOW_LIST copy = m_ChildWndList;
    ...
    for (auto it = copy.rbegin(); it != copy.rend(); ++it)
    {
        ...
        if (sw)
        {
            if (sw->GetAlignment() != waNone && sw->GetAlignment() != waLeft && sw->GetAlignment() != waRight && sw->GetAlignment() != waTop && sw->GetAlignment() != waBottom &&
                sw->GetAlignment() != waCenter)
                return false;
        }
...
bool CUIWindow::FillDebugTree(const CUIDebugState& debugState)
{
    ...
    const bool open = ImGui::TreeNodeEx("CUIWindow", flags, "%s (%s)", WindowName().c_str(), GetDebugType());
    ...
    if (debugState.drawWndRects && (IsShown() || hovered))
    {
        ...
        if (hovered)
            ...
            color = color_rgba(Random.randI(255), Random.randI(255), Random.randI(255), 255);
