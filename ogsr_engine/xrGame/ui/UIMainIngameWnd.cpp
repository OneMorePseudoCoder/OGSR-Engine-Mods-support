...
void CUIMainIngameWnd::Init()
{
    ...
    */	//---------------------------------------------------------
    ...
    if (m_ind_boost_satiety = UIHelper::CreateStatic(uiXml, "indicator_booster_satiety", this, false))
        m_ind_boost_satiety->Show(false);
    if (m_ind_boost_thirst = UIHelper::CreateStatic(uiXml, "indicator_booster_thirst", this, false))
...
void CUIMainIngameWnd::SetFlashIconState_(EFlashingIcons type, bool enable, bool fatal /*= true*/)
{
    ...
    switch (type)
    {
    case 0: iconType = "pda"; break;
    case 1: iconType = "encyclopedia"; break;
    case 2: iconType = "journal"; break;
    case 3: iconType = "mail"; break;
    default: Msg("! [%s]: Unknown flashing icon type: [%s]", __FUNCTION__, iconType.c_str());
    }

    if (icon == m_FlashingIcons.end())
    {
        if (fatal)
            R_ASSERT3(false, "Flashing icon with this type not existed!", iconType.c_str());
...
void CUIMainIngameWnd::DrawMainIndicatorsForInventory()
{
    ...
    if (m_ind_boost_satiety && m_ind_boost_satiety->IsShown())
    {
        m_ind_boost_satiety->Update();
        m_ind_boost_satiety->Draw();
    }

    if (m_ind_boost_thirst && m_ind_boost_thirst->IsShown())
    {
...
void CUIMainIngameWnd::UpdateBoosterIndicators(const xr_map<EBoostParams, SBooster> influences)
{
    ...
    m_ind_boost_power->Show(false);
    m_ind_boost_rad->Show(false);
    if (m_ind_boost_satiety)
        m_ind_boost_satiety->Show(false);
    if (m_ind_boost_thirst)
        m_ind_boost_thirst->Show(false);
...
#pragma todo("Pilot: подумать как заопционалить показ времени действия бустеров")
//             string16 buf = {};
