...
bool CUIInventoryUpgradeWnd::install_item(CInventoryItem& inv_item, bool can_upgrade)
{
    ...
    for (; ib != ie; ++ib)
    {
        ...
        for (u8 i = 0; i < inventory::upgrade::max_properties_count; i++)
        {
            ...
            if (prop_name.size())
            {
                VERIFY(get_manager().get_property(prop_name));
            }
        }
...
CUIInventoryUpgradeWnd::Manager_type& CUIInventoryUpgradeWnd::get_manager() { return ai().alife().inventory_upgrade_manager(); }