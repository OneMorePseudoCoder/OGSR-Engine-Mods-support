...
void CUICharacterInfo::InitCharacterInfo(Fvector2 pos, Fvector2 size, CUIXml* xml_doc)
{
    ...
    */
    ...
    Init_StrInfoItem(*xml_doc, "rank_caption", eRankCaption, eUIRankCaptionText);
    Init_StrInfoItem(*xml_doc, "rank_static", eRank);

    Init_StrInfoItem(*xml_doc, "community_caption", eCommunityCaption, eUICommunityCaptionText);
    Init_StrInfoItem(*xml_doc, "community_static", eCommunity);

    Init_StrInfoItem(*xml_doc, "reputation_caption", eReputationCaption, eUIReputationCaptionText);
    Init_StrInfoItem(*xml_doc, "reputation_static", eReputation);

    Init_StrInfoItem(*xml_doc, "relation_caption", eRelationCaption, eUIRelationCaptionText);
...
void CUICharacterInfo::Init_StrInfoItem(CUIXml& xml_doc, LPCSTR item_str, UIItemType type, UIItemCaption caption)
{
    if (xml_doc.NavigateToNode(item_str, 0))
    {
        ...
        if (caption < eMaxCaptionText)
        {
            xr_string capt_text{item_str};
            capt_text += ":text";
            m_text_ids[caption] = xml_doc.Read(capt_text.c_str(), 0, "");
        }
        AttachChild(pItem);
...
void CUICharacterInfo::InitCharacter(u16 id)
{
    ...
    if (m_icons[eName])
    {
        m_icons[eName]->TextItemControl()->SetText(T->name_translated().c_str());
    }
    if (m_icons[eRankCaption])
    {
        m_icons[eRankCaption]->TextItemControl()->SetTextST(m_text_ids[eUIRankCaptionText].c_str());
    }
    if (m_icons[eRank])
    {
    ...
    {
        m_icons[eRelationCaption]->TextItemControl()->SetTextST(m_text_ids[eUIRelationCaptionText].c_str());
    }

    if (m_icons[eCommunityCaption])
    {
        m_icons[eCommunityCaption]->TextItemControl()->SetTextST(m_text_ids[eUICommunityCaptionText].c_str());
    }
    if (m_icons[eCommunity])
    {
        m_icons[eCommunity]->TextItemControl()->SetTextST(chInfo.Community().id().c_str());
    }
    if (m_icons[eReputationCaption])
    {
        m_icons[eReputationCaption]->TextItemControl()->SetTextST(m_text_ids[eUIReputationCaptionText].c_str());
    }
    ...
    if (pUIBio && pUIBio->IsEnabled())
    {
        ...
        if (chInfo.Bio().size())
        {
            ...
            pItem->SetTextST(chInfo.Bio().c_str());
...
bool CUICharacterInfo::get_actor_community(shared_str* our, shared_str* enemy)
{
    ...
    __GetItem(vs_teams, 0, our_fract, size_temp);
    __GetItem(vs_teams, 1, enemy_fract, size_temp);
...
bool CUICharacterInfo::ignore_community(shared_str const& check_community)
{
    ...
    const auto& faction_section = pSettings->r_section(comm_section_str);
    for (const auto& comm_name : faction_section.Ordered_Data | std::views::keys)
    {
        if (check_community == comm_name)
        {
            return true;
        }
    }
