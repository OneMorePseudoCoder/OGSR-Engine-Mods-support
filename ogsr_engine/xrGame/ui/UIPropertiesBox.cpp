...
CUIPropertiesBox::~CUIPropertiesBox() { R_ASSERT2(!m_sub_property_box || (!m_sub_property_box->IsShown()), "child sub menu is in shown mode - he'll tries to hide this menu"); }
...
bool CUIPropertiesBox::AddItem(LPCSTR str, void* pData, u32 tag_value)
{
    ...
    if (m_sub_property_box)
    {
        AddCallback(itm, WINDOW_FOCUS_RECEIVED, fastdelegate::MakeDelegate(this, &CUIPropertiesBox::OnItemReceivedFocus));
        Register(itm);
    }
    return true;
}
