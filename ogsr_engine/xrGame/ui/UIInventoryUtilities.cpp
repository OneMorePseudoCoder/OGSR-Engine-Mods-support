    ...
    if (r1.x == r2.x)
    {
        ...
        if (r1.y == r2.y)
        {
            if (item1->object().cNameSect() == item2->object().cNameSect())
                ...
                return (item2->object().cNameSect() < item1->object().cNameSect());
        }
                    ...
                    if (!xr_strcmp(item1->object().cNameSect(), item2->object().cNameSect()))
                    {
                        ...
                        if (ammo1 && ammo2)
                        {
                            if (ammo1->m_boxCurr == ammo2->m_boxCurr)
                                // return (item1->object().ID() < item2->object().ID());
                                return xr_strcmp(item1->NameItem(), item2->NameItem()) < 0;
...
const shared_str InventoryUtilities::Get_GameTimeAndDate_AsString() { return GetTimeAndDateAsString(Level().GetGameTime()); }
...
const shared_str InventoryUtilities::GetTimeAsString(ALife::_TIME_ID time, ETimePrecision timePrec, char timeSeparator, bool full_mode)
{
    ...
    case etpTimeToSeconds:
        ...
        break;
    case etpTimeToMilisecs: xr_sprintf(bufTime, "%02i%c%02i%c%02i%c%02i", hours, timeSeparator, mins, timeSeparator, secs, timeSeparator, milisecs); break;
