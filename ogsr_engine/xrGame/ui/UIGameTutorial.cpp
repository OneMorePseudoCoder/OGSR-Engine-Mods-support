...
void CallFunction(shared_str const& func)
{
    ...
    ASSERT_FMT_DBG(functor_exists, "!!Cannot find script function described in tutorial item [%s]", func.c_str());
...
void CUISequenceItem::Start()
{
    ...
    if (m_onframe_lua_function.size())
    {
        ...
        ASSERT_FMT_DBG(functor_exists, "!!Cannot find script function described in tutorial item [%s]", m_onframe_lua_function.c_str());
    }
}
...
CUISequenceItem* CUISequencer::GetNextItem()
{
    ...
    while (m_sequencer_items.size())
    {
        ...
        ASSERT_FMT_DBG(functor_exists, "!!Cannot find script function described in tutorial item [%s]", f.c_str());
