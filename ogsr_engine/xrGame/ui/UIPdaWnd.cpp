...
#pragma todo("Pilot: подумать как можно заопционалить это, потому что в оригинале мигающая иконка для этого есть.")
        //CurrentGameUI()->UIMainIngameWnd->SetFlashIconState_(CUIMainIngameWnd::efiPdaTask, false);
        CurrentGameUI()->UIMainIngameWnd->SetFlashIconState_(CUIMainIngameWnd::efiEncyclopedia, false, false);
...
void CUIPdaWnd::Update()
{
    ...
    Device.add_to_seq_parallel(fastdelegate::MakeDelegate(pUILogsWnd, &CUILogsWnd::PerformWork));
}
...
void CUIPdaWnd::PdaContentsChanged(pda_section::part type)
{
    if (type == pda_section::encyclopedia)
    {
        ...
        CurrentGameUI()->UIMainIngameWnd->SetFlashIconState_(CUIMainIngameWnd::efiEncyclopedia, true, false);
    }
#pragma todo("Pilot: подумать как можно заопционалить это, потому что в оригинале мигающая иконка для этого есть.")
