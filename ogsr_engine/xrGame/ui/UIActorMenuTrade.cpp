...
void CUIActorMenu::UpdateActor()
{
    ...
    m_ActorMoney->SetText(std::format("{} {}", m_pActorInvOwner->get_money(), StMoneyDescr).c_str());
...
{
    static const char* StMoneyDescr = CStringTable().translate("ui_st_money_descr").c_str();
    auto* monster = smart_cast<CBaseMonster*>(m_pPartnerInvOwner);
    ...
    {
        m_PartnerMoney->SetText(std::format("--- {}", StMoneyDescr).c_str());
    }
    else
    {
        m_PartnerMoney->SetText(std::format("{} {}", m_pPartnerInvOwner->get_money(), StMoneyDescr).c_str());
    }

    LPCSTR kg_str = CStringTable().translate("st_kg").c_str();
    float total = CalcItemsWeight(m_pTradePartnerBagList);
    m_PartnerWeight->SetText(std::format("{:.1f} {}", total, kg_str).c_str());
...
void CUIActorMenu::UpdatePrices()
{
    ...
    m_ActorTradePrice->SetText(std::format("{} {}", actor_price, StMoneyDescr).c_str());
    m_ActorTradePrice->AdjustWidthToText();
    m_PartnerTradePrice->SetText(std::format("{} {}", partner_price, StMoneyDescr).c_str());
    ...
    m_ActorTradeWeightMax->SetText(std::format("({:.1f} {})", actor_weight, kg_str).c_str());
    m_PartnerTradeWeightMax->SetText(std::format("({:.1f} {})", partner_weight, kg_str).c_str());
...
void CUIActorMenu::OnBtnPerformTradeBuy(CUIWindow* w, void* d)
{
    ...
    partner_money -= (m_pPartnerInvOwner->InfinitiveMoney()) ? 0 : delta_price;
