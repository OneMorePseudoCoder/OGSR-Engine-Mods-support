...
bool CUITextureMaster::IsSh(const shared_str& texture_name) { return strstr(texture_name.c_str(), "\\") ? false : true; }
...
void CUITextureMaster::GetTextureShader(const shared_str& texture_name, ui_shader& sh)
{
    ...
    R_ASSERT3(it != m_textures.end(), "can't find texture %s", texture_name.c_str());
