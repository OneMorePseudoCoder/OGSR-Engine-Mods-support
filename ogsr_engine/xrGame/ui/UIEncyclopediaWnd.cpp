...
{
    CUIXml uiXml;
    if (!uiXml.Load(CONFIG_PATH, UI_PATH, ENCYCLOPEDIA_DIALOG_XML, false))
        return;
    ...
    UIBackground = UIHelper::CreateFrameLine(uiXml, "background", this, false);

    UIEncyclopediaIdxBkg = UIHelper::CreateFrameLine(uiXml, "left_background", this, false);
    UIEncyclopediaIdxHeader = UIHelper::CreateStatic(uiXml, "left_caption", UIEncyclopediaIdxBkg, false);

    UIEncyclopediaInfoBkg = UIHelper::CreateFrameLine(uiXml, "right_background", this, false);
    UIEncyclopediaInfoHeader = UIHelper::CreateStatic(uiXml, "right_caption", UIEncyclopediaInfoBkg, false);
...
{
    if (UIIdxList && UIIdxList == pWnd && LIST_ITEM_CLICKED == msg)
    {
...
void CUIEncyclopediaWnd::SetCurrentArtice(CUITreeViewItem* pTVItem)
{
    ...
    // для начала проверим, что нажатый элемент не корневой
    if (!pTVItem->IsRoot())
    {
...
void CUIEncyclopediaWnd::AddArticle(shared_str article_id, bool bReaded)
{
    ...
    // Теперь создаем иерархию ветвей по заданному пути
