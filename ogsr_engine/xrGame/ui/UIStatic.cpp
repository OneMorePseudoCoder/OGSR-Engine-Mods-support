...
CUIStatic::CUIStatic() : m_bTextureEnable(true), m_bStretchTexture(false), m_bHeading(false), m_bConstHeading(false), m_fHeading(0.0f), m_pTextControl(nullptr), m_bClipper(false)
{
...
void CUIStatic::Update()
{
    ...
    if (CursorOverWindow() && m_stat_hint_text.size() && !g_statHint->Owner() &&
        (MainMenu() && !MainMenu()->IsActive() ? (Device.dwTimeGlobal > m_dwFocusReceiveTime + 700) : true))
    {
...
void CUIStatic::ClipperOff(CUIStaticItem& UIStaticItem)
{
    ...
    Frect r{};
    r.x1 = out_rect.left;
    r.x2 = out_rect.right < UIStaticItem.GetTextureRect().width() ? out_rect.right : UIStaticItem.GetTextureRect().width();

    r.y1 = out_rect.top;
    r.y2 = out_rect.bottom < UIStaticItem.GetTextureRect().height() ? out_rect.bottom : UIStaticItem.GetTextureRect().height();
    UIStaticItem.SetSize(Fvector2().set(r.width(), r.height()));
}
...
void CUIStatic::ColorAnimationSetTextureColor(u32 color, bool only_alpha) { SetTextureColor((only_alpha) ? subst_alpha(GetTextureColor(), color) : color); }
...
void CUIStatic::SetHint(LPCSTR hint_text) { m_stat_hint_text = hint_text; }
...
void CUITextWnd::ColorAnimationSetTextColor(u32 color, bool only_alpha) { SetTextColor((only_alpha) ? subst_alpha(GetTextColor(), color) : color); }
