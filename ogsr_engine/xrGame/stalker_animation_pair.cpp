...
void CStalkerAnimationPair::play_global_animation(IKinematicsAnimated* skeleton_animated, PlayCallback callback, const u32& bone_part, const bool& use_animation_movement_control,
...
#ifndef USE_HEAD_BONE_PART_FAKE
void CStalkerAnimationPair::play(IKinematicsAnimated* skeleton_animated, PlayCallback callback, const bool& use_animation_movement_control, const bool& local_animation,
                                 bool continue_interrupted_animation, bool mix_animations)
#else
void CStalkerAnimationPair::play(IKinematicsAnimated* skeleton_animated, PlayCallback callback, const bool& use_animation_movement_control, const bool& local_animation,
...
#endif
{
    ...
    if (psAI_Flags.is(aiAnimation))
    {
        ...
        Msg("%6d [%s][%s][%s][%d][%c][%c][%c][%f][%f][%f]", Device.dwTimeGlobal, m_object_name, m_animation_type_name, name, motion->bone_or_part,
            (!(motion->flags & esmStopAtEnd)) ? '+' : '-', use_animation_movement_control ? '+' : '-', local_animation ? '+' : '-',
