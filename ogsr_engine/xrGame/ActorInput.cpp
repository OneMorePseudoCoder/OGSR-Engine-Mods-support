    ...
    if (!m_pUsableObject || m_pUsableObject->nonscript_usable())
    {
        if (m_pPersonWeLookingAt)
        {
            ...
            else
            {
                ...
                if (pGameSP)
                {
                    if (!m_pPersonWeLookingAt->deadbody_closed_status() && m_pPersonWeLookingAt->cast_inventory_owner())
                    {
                        if (pEntityAliveWeLookingAt->AlreadyDie() && pEntityAliveWeLookingAt->GetLevelDeathTime() + 1000 < Device.dwTimeGlobal)
                            // 99.9% dead
        ...
        if (object && pInput->iGetAsyncKeyState(DIK_LSHIFT))
        {
