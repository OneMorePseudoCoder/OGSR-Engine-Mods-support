...
void CImGuiPropEditorWnd::Render()
{
    if (!RenderBegin())
    {
        RenderEnd();
    ...
    if (object)
    {
        ...
        if (Actor()->Position().distance_to(object->Position()) > MAX_DIST)
            object = nullptr;
    }
    ...
    ImGui::Text("Selected: %s", object ? object->cName().c_str() : "None");

    if (ImGui::Button("Capture by Look"))
    {
        ...
        if (g_pGameLevel && g_pGameLevel->ObjectSpace.RayPick(start, dir, max_distance, collide::rqtObject, RQ, nullptr))
        {
            if (RQ.O)
            {
                CEntityAlive* alive = smart_cast<CEntityAlive*>(RQ.O);
                if (alive && alive->g_Alive() && !smart_cast<CActor*>(alive))
                {
                    object = alive;
                }
            }
    ...
    if (ImGui::Button("Remove Capture"))
    {
        object = nullptr;
    }
    ...
    if (gameObject && gameObject->object())
    {
        ...
        if (ImGui::CollapsingHeader("CScriptBinder"))
        {
            ...
            ImGui::SameLine();
            if (ImGui::Button("..."))
                ImGui::OpenPopup("Lua Binder");

            if (ImGui::BeginPopupModal("Lua Binder", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
            {
                CGameObject* gameObject = smart_cast<CGameObject*>(object);
                ...
                else
                {
                    ...
                    luabind::wrap_base* base = static_cast<CScriptBinderObjectWrapper*>(gameObject->object());
                    ...
                    binder.pushvalue();
                    auto* obj = static_cast<luabind::detail::object_rep*>(lua_touserdata(binder.lua_state(), -1));
                    ...
                    ImGui::InputTextMultiline("##lua-binder-content", &content[0], content.size(), ImVec2(700.0f, 500.0f), ImGuiInputTextFlags_ReadOnly);
                }
            ...
            binder.pushvalue();
            luabind::detail::object_rep* obj = static_cast<luabind::detail::object_rep*>(lua_touserdata(binder.lua_state(), -1));
            lua_pop(binder.lua_state(), 1);
            luabind::object luaBinder(binder.lua_state(), obj->get_lua_table(), true);
