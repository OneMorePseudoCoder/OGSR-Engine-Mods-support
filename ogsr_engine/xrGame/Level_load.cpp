...
BOOL CLevel::Load_GameSpecific_After()
{
    ...
    {
        const auto& S = pSettings->r_section("sounds_random");
        Sounds_Random.reserve(S.Ordered_Data.size());
        for (const auto& name : S.Ordered_Data | std::views::keys)
        {
            Sound->create(Sounds_Random.emplace_back(), name.c_str(), st_Effect, sg_SourceType);
        }
...
/*
void CLevel::Load_GameSpecific_CFORM_Serialize(IWriter& writer) { writer.w_u32(GMLib.GetFileAge()); }
