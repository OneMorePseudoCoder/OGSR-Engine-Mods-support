...
{
    m_entity_condition = ec ? smart_cast<CEntityCondition*>(ec) : xr_new<CEntityConditionSimple>();
...
BOOL CEntity::net_Spawn(CSE_Abstract* DC)
{
    ...
    if (E)
    {
        ...
        R_ASSERT2(!((E->get_killer_id() != ALife::_OBJECT_ID(-1)) && g_Alive()),
                  make_string("server entity [%s][%d] has an killer [%d] and not dead", E->name_replace(), E->ID, E->get_killer_id()).c_str());
