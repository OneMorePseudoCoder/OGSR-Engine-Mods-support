...
inline CFixedVertexManager::CDataStorage(const u32 vertex_count) : CDataStorageBase(vertex_count), CDataStorageAllocator()
{
...
TEMPLATE_SPECIALIZATION
inline bool CFixedVertexManager::is_closed(const Vertex& vertex) const { return is_visited(vertex) && !is_opened(vertex); }
