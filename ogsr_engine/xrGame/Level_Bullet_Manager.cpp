...
void SBullet::Init(const Fvector& position, const Fvector& direction, float starting_speed, float power, float impulse, u16 sender_id, u16 sendersweapon_id,
                   ALife::EHitType e_hit_type, float maximum_distance, const CCartridge& cartridge, float const air_resistance_factor, bool SendHit)
{
    flags._storage = 0;
    bullet_pos = position;
...
void CBulletManager::AddBullet(const Fvector& position, const Fvector& direction, float starting_speed, float power,
                               ...
                               float impulse, u16 sender_id, u16 sendersweapon_id, ALife::EHitType e_hit_type, float maximum_distance, const CCartridge& cartridge,
                               float const air_resistance_factor, bool SendHit, bool AimBullet)
{
    ...
    bullet.Init(position, direction, starting_speed, power, /*power_critical,*/ impulse, sender_id, sendersweapon_id, e_hit_type, maximum_distance, cartridge,
...
{
    VERIFY((bullet.max_dist - bullet.fly_dist) > 0.f);
...
void CBulletManager::add_bullet_point(Fvector const& start_position, Fvector& previous_position, Fvector const& start_velocity, Fvector const& gravity, float const air_resistance,
...
bool CBulletManager::trajectory_check_error(Fvector& previous_position, collide::rq_results& storage, SBullet& bullet, float& low, float& high, Fvector const& gravity,
