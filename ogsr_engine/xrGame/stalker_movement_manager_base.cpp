...
stalker_movement_manager_base::stalker_movement_manager_base(CAI_Stalker* object)
    : inherited(object), m_last_query_object(0), m_last_query_position(Fvector().set(flt_max, flt_max, flt_max)),
      m_last_query_object_position(Fvector().set(flt_max, flt_max, flt_max)), m_last_query_result(false), m_last_query_distance(flt_max), m_force_update(false)
{
    VERIFY(object);
    m_object = object;
    m_velocities = 0;
...
void stalker_movement_manager_base::init_velocity_masks()
{
    ...
    add_velocity(eVelocityWalkFreePositive, m_velocities->velocity(eMentalStateFree, eBodyStateStand, eMovementTypeWalk, eMovementDirectionForward), PI * 100.f / 180.f,
                 cf * PI * 100.f / 180.f);
...
void stalker_movement_manager_base::setup_movement_params(stalker_movement_params& movement_params)
{
    ...
    {
        if ((movement_params.m_path_type != MovementManager::ePathTypePatrolPath) && (movement_params.m_path_type != MovementManager::ePathTypeGamePath) &&
            (movement_params.m_path_type != MovementManager::ePathTypeNoPath))
        {
            ...
            else
            {
                ...
                VERIFY2(restrictions().accessible(vertex_position) || show_restrictions(&restrictions()),
                        make_string("vertex_id[%d],position[%f][%f][%f],object[%s]", level_path().dest_vertex_id(), VPUSH(vertex_position), *object().cName()));
...
void stalker_movement_manager_base::set_nearest_accessible_position() { set_nearest_accessible_position(object().Position(), object().ai_location().level_vertex_id()); }
...
void stalker_movement_manager_base::on_travel_point_change(const u32& previous_travel_point_index) { inherited::on_travel_point_change(previous_travel_point_index); }
...
void stalker_movement_manager_base::setup_speed_from_animation(const float& speed) { set_desirable_speed(object().m_fCurSpeed = speed); }
