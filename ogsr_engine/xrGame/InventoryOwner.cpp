...
void CInventoryOwner::spawn_supplies()
{
    ...
    static const char* bolt_section = READ_IF_EXISTS(pSettings, r_string, "default_items", "bolt_section", "bolt");
    static const char* pda_section = READ_IF_EXISTS(pSettings, r_string, "default_items", "pda_section", "device_pda");

    if (use_bolts())
        Level().spawn_item(bolt_section, game_object->Position(), game_object->ai_location().level_vertex_id(), game_object->ID());

    if (!ai().get_alife())
    {
        CSE_Abstract* abstract = Level().spawn_item(pda_section, game_object->Position(), game_object->ai_location().level_vertex_id(), game_object->ID(), true);
...
{
    auto* object = smart_cast<CGameObject*>(this);
    VERIFY(object);
...
CCustomOutfit* CInventoryOwner::GetOutfit() const { return smart_cast<CCustomOutfit*>(inventory().ItemFromSlot(OUTFIT_SLOT)); }
