...
void CSpaceRestriction::merge_in_out_restrictions()
{
    ...
    {
        buffer_vector<u32> temp_border(_alloca(m_in_space_restriction->border().size() * sizeof(u32)), m_in_space_restriction->border().size(),
...
CSpaceRestriction::CBaseRestrictionPtr CSpaceRestriction::merge(CBaseRestrictionPtr bridge, const RESTRICTIONS& temp_restrictions) const
{
    ...
    for (const auto& it : temp_restrictions)
    {
        temp = xr_strconcat(tempBuffer, *temp, ",", it->name().c_str());
    }
    */

    //SIMP: этот вариант выглядит надежнее
    ...
    string4096 S{};
    R_ASSERT(acc_length < std::size(S)); // переделать если такое будет
    shared_str temp = bridge->name();
    for (const auto& it : temp_restrictions)
        temp = xr_strconcat(S, *temp, ",", it->name().c_str());
