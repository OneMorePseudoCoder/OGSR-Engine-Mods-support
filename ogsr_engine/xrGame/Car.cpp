...
void CCar::cb_Steer(CBoneInstance* B)
{
    ...
    {
        Msg("RotatingZ angle=", C->m_steer_angle);
...
void CCar::CreateSkeleton(CSE_Abstract* po)
{
    ...
    {
        pKA->PlayCycle("idle");
        pK->CalculateBones(/*TRUE*/); // у нас нету тут true
    }
...
void CCar::Init()
{
    ...
    {
        const auto& data = ini->r_section("damage_items");
        for (const auto& [key, val] : data.Ordered_Data)
        {
            u16 index = pKinematics->LL_BoneID(key);
            R_ASSERT3(index != BI_NONE, "Wrong bone name", key.c_str());
            auto i = m_wheels_map.find(index);

            if (i != m_wheels_map.end())
                i->second.CDamagableHealthItem::Init(float(atof(val.c_str())), 2);
            else
            {
                auto i = m_doors.find(index);
                R_ASSERT3(i != m_doors.end(), "only wheel and doors bones allowed for damage defs", key.c_str());
                i->second.CDamagableHealthItem::Init(float(atof(val.c_str())), 1);
            }
        }
    }
