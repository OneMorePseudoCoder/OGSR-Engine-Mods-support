...
void CGameObject::OnEvent(NET_Packet& P, u16 type)
{
    ...
    case GE_DESTROY: {
        if (H_Parent())
        {
            Msg("! ERROR (GameObject): GE_DESTROY arrived to object[%d][%s], that has parent[%d][%s], frame[%d]", ID(), cNameSect().c_str(), H_Parent()->ID(),
    ...
    if (visual)
    {
        ...
        if (pSettings->line_exist(cNameSect(), "visual"))
        {
            ...
            if (!strext(*config_visual))
                xr_strconcat(config_visual_file, *config_visual, ".ogf");
            else
            ...
            if (!strext(saved_visual))
                xr_strconcat(saved_visual_file, saved_visual, ".ogf");
            else
...
CGameObject::CScriptCallbackExVoid& CGameObject::callback(GameObject::ECallbackType type) const { return ((*m_callbacks)[type]); }
...
bool CGameObject::animation_movement_controlled() const { return !!animation_movement() && animation_movement()->IsActive(); }
...
void CGameObject::create_anim_mov_ctrl(CBlend* b, Fmatrix* start_pose, bool local_animation)
{
    ...
    else
    {
        ...
        VERIFY2(start_pose,
                make_string("start pose hasn't been specified for animation [%s][%s]", smart_cast<IKinematicsAnimated&>(*Visual()).LL_MotionDefName_dbg(b->motionID).first,
