...
Fvector CCustomMonster::predict_position(const float& time_to_check) const { return (movement().predict_position(time_to_check)); }
...
const Fvector& CCustomMonster::spatial_sector_point()
{
    sector_point_saved = inherited::spatial_sector_point();
    return sector_point_saved.add({0.f, Radius() * .5f, 0.f});
}
