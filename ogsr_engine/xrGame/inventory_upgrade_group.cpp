...
void Group::construct(const shared_str& group_id, UpgradeBase& parent_upgrade, Manager& manager_r)
{
    ...
    {
        UpgradeBase* upgrade_p = (UpgradeBase*)manager_r.add_upgrade(__GetItem(upgrades_str, i, temp, buffer_size), *this);
...
UpgradeStateResult Group::can_install(CInventoryItem& item, UpgradeBase& test_upgrade, bool loading)
{
    ...
    for (; ib != ie; ++ib)
    {
        ...
        if (!item.has_upgrade_group(U->parent_group_id()))
        {
            if (loading)
            {
                FATAL(make_string("Loading item: Upgrade <%s> of inventory item [%s] (id = %d) can`t be installed! Error = result_e_parents", test_upgrade.id_str(),
    ...
    for (; ib != ie; ++ib)
    {
        ...
        if (item.has_upgrade((*ib)->id()))
        {
            if (loading)
            {
                FATAL(make_string("Loading item: Upgrade <%s> of inventory item [%s] (id = %d) can`t be installed! Error = result_e_group", test_upgrade.id_str(),
                                  item.m_section_id.c_str(), item.object_id())
                          .c_str());
            }
