...
TEMPLATE_SPECIALIZATION
inline typename CHashFixedVertexManager::PathId CHashFixedVertexManager::current_path_id() const { return m_current_path_id; }
...
{
    // Pilot: костыль из IX-Ray, на удивление, работает, но надо будет переделать, основная проблема в том, что
    // сюда попадает const shared_str, откуда - не ясно.
    return (MakeMeUnsigned(vertex_id) % HashSize);
...
TEMPLATE_SPECIALIZATION
inline typename CHashFixedVertexManager::Vertex& CHashFixedVertexManager::create_vertex(Vertex& vertex, const Index& vertex_id)
{
