...
m_tpaCombatGroups[iCombatGroupIndex][0]->m_tpBestDetector; ai().ef_storage().alife().member()	= m_tpaCombatGroups[iCombatGroupIndex][0]; ai().ef_storage().alife().enemy() =
m_tpaCombatGroups[iCombatGroupIndex ^ 1][0]; return										(randF(100) < ((eCombatTypeMonsterMonster != combat_type()) ?
ai().ef_storage().m_pfAnomalyRetreatProbability->ffGetValue() : ai().ef_storage().m_pfEnemyRetreatProbability->ffGetValue()));
}
