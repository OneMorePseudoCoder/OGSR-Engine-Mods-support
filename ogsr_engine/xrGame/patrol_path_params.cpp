                                     ...
                                     bool bRandom, u32 index)
{
    ...
    ASSERT_FMT_DBG(m_path, "There is no patrol path [%s]", caPatrolPathToGo);
...
{
    ASSERT_FMT_DBG(m_path, "!![%s] There is no patrol path [%s]", __FUNCTION__, m_path_name);

    if (m_path)
        if (const auto* vert = m_path->vertex(index))
            return Flags32().assign(vert->data().flags());
    return {};
}
