...
stalker_movement_manager_smart_cover::stalker_movement_manager_smart_cover(CAI_Stalker* object)
    : inherited(object), m_property_storage(0), m_current_transition(0), m_current_transition_animation(0), m_non_animated_loophole_change(false),
      m_apply_loophole_direction_distance(4.f), m_animation_selector(0), m_entering_smart_cover_with_animation(false), m_default_behaviour(false), m_enter_cover_id(""),
...
void stalker_movement_manager_smart_cover::enter_smart_cover()
{
    ...
    if (!m_current.cover() && (m_enter_cover_id != "") && ((m_target.cover_id() != m_enter_cover_id) || (shared_str(m_target.cover_loophole_id()) != m_enter_loophole_id)))
    {
#ifdef DEBUG
...
void stalker_movement_manager_smart_cover::on_frame(CPHMovementControl* movement_control, Fvector& dest_position) { inherited::on_frame(movement_control, dest_position); }
...
void stalker_movement_manager_smart_cover::loophole_path(smart_cover::cover const& cover, shared_str const& source_raw, shared_str const& target_raw, LoopholePath& path) const
{
    ...
    R_ASSERT2(ai().graph_engine().search(cover.description()->transitions(), source, target, &path, parameters),
              make_string("cannot build path via loopholes [%s] -> [%s] (cover %s)", source_raw.c_str(), target_raw.c_str(), cover.description()->table_id().c_str()));
}
...
{
#ifdef DEBUG
    Msg("m_current_transition guard: [%s][%s] -> [%s][%s], [%d]", m_current.cover() ? m_current.cover()->id().c_str() : "<world>",
