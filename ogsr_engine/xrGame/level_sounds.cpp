    ...
    if (gameLtx.section_exist(Level().name()))
    {
        if (gameLtx.line_exist(Level().name(), "music_tracks"))
        {
            ...
            if (music_sect && music_sect[0])
            {
                ...
                m_MusicTracks.reserve(S.Ordered_Data.size());
                for (const auto& [key, val] : S.Ordered_Data)
                {
                    m_MusicTracks.emplace_back().Load(key.c_str(), val.c_str());
                }
            }
        }
    }
    ...
    if (!m_MusicTracks.empty())
    {
        ...
        if (m_CurrentTrack < 0 && engine_time > m_NextTrackTime)
        {
            ...
            if (!indices.empty())
            {
                ...
                T.Play();
#ifdef DEBUG
                Msg("- Play music track:", T.m_DbgName.c_str());
#endif
            }
            else
            {
                m_NextTrackTime = engine_time + m_NextTrackTimeInterval;
            }
        }
        ...
        if (m_CurrentTrack >= 0)
        {
            ...
            if (!T.IsPlaying())
            {
                ...
                if (TrackTimeIntervalUsePause && !((0 == T.m_PauseTime.x) && (0 == T.m_PauseTime.y)))
