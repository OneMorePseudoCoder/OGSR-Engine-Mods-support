...
void CActor::g_fireParams(CHudItem* pHudItem, Fvector& fire_pos, Fvector& fire_dir, const bool for_cursor)
{
    ...
    if (auto pMissile = smart_cast<CMissile*>(pHudItem); pMissile && !for_cursor)
    {
        Fvector offset;
    ...
    {
        // SIMP: Пусть пули летят из ствола даже в прицеливании, чарли сказал что в ЧН так надо, думаю тут тоже
        if (psHUD_Flags.test(HUD_CROSSHAIR_HARD) && weapon->need_renderable() /*!(weapon->IsZoomed() && !weapon->IsRotatingToZoom())*/)
        {
