...
void CParticlesPlayer::LoadParticles(IKinematics* K)
{
    ...
    {
        bone_mask = 0;
        const auto& data = ini->r_section("particle_bones");
        for (const auto& [key, val] : data.Ordered_Data)
        {
            u16 index = K->LL_BoneID(key);
            R_ASSERT3(index != BI_NONE, "Particles bone not found", key.c_str());
            Fvector offs{};
            sscanf(val.c_str(), "%f,%f,%f", &offs.x, &offs.y, &offs.z);
...
struct SRP
{
    bool operator()(const CParticlesPlayer::SParticlesInfo& pi) const { return !pi.ps; }
};
