...
float CHUDTarget::GetDist() { return PP.RQ.range; }

CObject* CHUDTarget::GetObj() { return PP.RQ.O; }
...
ICF static BOOL pick_trace_callback(collide::rq_result& result, LPVOID params)
{
    ...
    else
    {
        ...
        if (fsimilar(mtl->fVisTransparencyFactor, 1.0f, EPS) && fsimilar(mtl->fShootFactor, 1.0f, EPS) && mtl->Flags.is(SGameMtl::flSuppressWallmarks))
            return TRUE;
    }
    pp->RQ = result;
    return FALSE;
...
void CHUDTarget::Render()
{
    ...
    bool need_to_show_cursor = psHUD_Flags.is(HUD_CROSSHAIR | HUD_CROSSHAIR_RT | HUD_CROSSHAIR_RT2);
    ...
    if (const auto Wpn = smart_cast<CWeapon*>(active_item); Wpn && (/*Wpn->IsLaserOn() ||*/ Wpn->GetState() == CWeapon::EWeaponStates::eReload))
        need_to_show_cursor = false;
