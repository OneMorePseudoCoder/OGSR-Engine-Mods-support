...
public:
    ...
    IC bool operator==(const _vertex_id_type& vertex_id) const { return (this->vertex()->vertex_id() == vertex_id); }
};

template <typename _edge_weight_type, typename _vertex_type>
class CEdge<_edge_weight_type, _vertex_type, std::monostate> : public CEdgeBase<_edge_weight_type, _vertex_type>
{
private:
    typedef CEdgeBase<_edge_weight_type, _vertex_type> inherited;
    using _vertex_id_type = inherited::_vertex_id_type;
