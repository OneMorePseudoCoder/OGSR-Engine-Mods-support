...
CPHMovementControl::CPHMovementControl(CObject* parent)
{
    ...
    {
        Msg("CPHMovementControl::CPHMovementControl %s (constructor) %f,%f,%pObjectf", debug_output().PH_DBG_ObjectTrackName(), pObject->Position().x, pObject->Position().y,
            pObject->Position().z);
    }
...
void CPHMovementControl::Calculate(const xr_vector<DetailPathManager::STravelPathPoint>& in_path, float speed, u32& travel_point, float& precision)
{
    ...
    if (debug_output().ph_dbg_draw_mask1().test(ph_m1_DbgTrackObject) && (!!pObject->cName()) && _stricmp(debug_output().PH_DBG_ObjectTrackName(), *pObject->cName()) == 0)
    {
        Msg("CPHMovementControl::Calculate in %s (Object Position) %f,%f,%f", debug_output().PH_DBG_ObjectTrackName(), pObject->Position().x, pObject->Position().y,
            pObject->Position().z);
...
void CPHMovementControl::AllocateCharacterObject(CharacterType type)
{
    ...
    if (ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject) && (!!pObject->cName()) && _stricmp(PH_DBG_ObjectTrackName(), *pObject->cName()) == 0)
    {
        Msg("CPHMovementControl::AllocateCharacterObject %s (Object Position) %f,%f,%f", PH_DBG_ObjectTrackName(), pObject->Position().x, pObject->Position().y,
            pObject->Position().z);
...
bool CPHMovementControl::IsCharacterEnabled() { return m_character->IsEnabled() || bExernalImpulse || bNonInteractiveMode; }
...
bool CPHMovementControl::JumpState() { return (m_character && m_character->b_exist && m_character->IsEnabled() && m_character->JumpState()); }
///
