...
void dump_collision_hit(CPHMovementControl* movement_control)
{
    ...
    Msg("ai unit: %s hited by collision; power: %f, spawn frame %d, current frame %d ", obj->cName().c_str(), movement_control->gcontact_HealthLost, obj->spawn_time(),
        Device.dwFrame);
...
void CMovementManager::move_along_path(CPHMovementControl* movement_control, Fvector& dest_position, float time_delta)
{
    ...
    //. 	VERIFY2(movement_control->CharacterExist() || object().animation_movement_controlled() , "! Can not move - physics movement shell does not exist. Try to move in wonded
    // state?" );
    if (!movement_control->CharacterExist())
        return;
