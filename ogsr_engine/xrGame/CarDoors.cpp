...
void CCar::SDoor::Init()
{
    ...
    if (!joint)
        return;
    // R_ASSERT2(dJointGetType(joint->GetDJoint())==dJointTypeHinge,"Wrong door joint!!! Only simple joint valid for a door and only one axis can be active, check other axes are
    // zerro limited !!!");
