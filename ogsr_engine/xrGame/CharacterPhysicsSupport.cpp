...
CCharacterPhysicsSupport::CCharacterPhysicsSupport(EType atype, CEntityAlive* aentity)
    : m_pPhysicsShell(aentity->PPhysicsShell()), m_EntityAlife(*aentity), mXFORM(aentity->XFORM()), m_ph_sound_player(aentity), m_interactive_motion(0),
      m_PhysicMovementControl(xr_new<CPHMovementControl>(aentity)), m_eType(atype), m_eState(esAlive), m_physics_skeleton(nullptr), m_ik_controller(nullptr),
      m_BonceDamageFactor(1.f), m_collision_hit_callback(nullptr), m_interactive_animation(nullptr), m_physics_shell_animated(nullptr),
      m_physics_shell_animated_time_destroy(u32(-1)), m_weapon_attach_bone(0), m_active_item_obj(0), m_hit_valide_time(u32(-1)), m_collision_activating_delay(nullptr)
{
    m_flags.assign(0);
...
bool CCharacterPhysicsSupport::CollisionCorrectObjPos() { return CollisionCorrectObjPos(m_EntityAlife.Position(), true); }
...
void CCharacterPhysicsSupport::SpawnInitPhysics(CSE_Abstract* e)
{
    if (m_EntityAlife.g_Alive())
    {
        ...
        {
            Msg("CCharacterPhysicsSupport::SpawnInitPhysics obj %s before collision correction %f,%f,%f", PH_DBG_ObjectTrackName(), m_EntityAlife.Position().x,
        ...
        if (ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject) && _stricmp(PH_DBG_ObjectTrackName(), *m_EntityAlife.cName()) == 0)
        {
            Msg("CCharacterPhysicsSupport::SpawnInitPhysics obj %s after collision correction %f,%f,%f", PH_DBG_ObjectTrackName(), m_EntityAlife.Position().x,
                m_EntityAlife.Position().y, m_EntityAlife.Position().z);
        }
#endif
    }
...
// }
bool CCharacterPhysicsSupport::has_shell_collision_place(const CPhysicsShellHolder* obj) const { return m_active_item_obj && obj == m_active_item_obj; }
...
void CCharacterPhysicsSupport::CreateShell(CObject* who, Fvector& dp, Fvector& velocity)
{
    ...
    if (!READ_IF_EXISTS(pSettings, r_bool, "gunslinger_base", "enable_corpse_collision", false))
    {
...
void CCharacterPhysicsSupport::EndActivateFreeShell(CObject* who, const Fvector& inital_entity_position, const Fvector& dp, const Fvector& velocity)
{
    ...
    // if( false &&  anim_mov_ctrl && anim_mov_blend && anim_mov_blend->blend != CBlend::eFREE_SLOT &&  anim_mov_blend->timeCurrent + Device.fTimeDelta*anim_mov_blend->speed <
...
bool CCharacterPhysicsSupport::can_drop_active_weapon() { return !interactive_motion() && m_flags.test(fl_death_anim_on); };
