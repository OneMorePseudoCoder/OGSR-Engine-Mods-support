...
LPCSTR CPhraseDialog::GetPhraseText(const shared_str& phrase_id, bool current_speaking)
{
    ...
    if (ph->m_script_text_id.length() > 0)
    {
        ...
        ASSERT_FMT_DBG(functor_exists, "Cannot find function: [%s]", ph->m_script_text_id.c_str());
...
void CPhraseDialog::load_shared(LPCSTR)
{
    ...
    if (nullptr == phrase_list_node)
    {
        ...
        ASSERT_FMT_DBG(functor_exists, "Cannot find precondition: [%s]", func);
        lua_function(this);
        return;
    }

    int phrase_num = pXML->GetNodesNum(phrase_list_node, "phrase");
    ASSERT_FMT_DBG(phrase_num, "dialog %s has no phrases at all", *item_data.id);
