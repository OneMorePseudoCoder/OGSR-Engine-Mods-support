    ...
    if (Visual() && smart_cast<IKinematics*>(Visual()))
    {
        ...
        if (PKinematicsAnimated)
        {
            ...
            VERIFY2((!!PKinematicsAnimated->LL_MotionID(visual->startup_animation.c_str()).valid()),
                    (make_string(" animation %s not faund ", visual->startup_animation.c_str()) + dbg_object_base_dump_string(this)).c_str());
...
void CPhysicObject::anim_time_set(float time)
{
    ...
    {
#ifdef DEBUG
        Msg(" ! can not set blend time %f - it must be in range 0 - %f(timeTotal) obj: %s, model: %s, anim: %s", time, m_anim_blend->timeTotal, cName().c_str(),
            cNameVisual().c_str(), smart_cast<IKinematicsAnimated*>(PPhysicsShell()->PKinematics())->LL_MotionDefName_dbg(m_anim_blend->motionID).first);
#endif
        return;
    }
...
void CPhysicObject::net_Export(NET_Packet& P) { P.w_u8(0); };
...
void CPhysicObject::Interpolate() { return; }
...
bool CPhysicObject::get_door_vectors(Fvector& closed, Fvector& open) const
{
    ...
    // fix for sliding doors from OpenXRay
