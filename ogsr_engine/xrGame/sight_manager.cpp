...
void CSightManager::Exec_Look(float time_delta)
{
    ...
    m_object->angle_lerp_bounds(body.current.yaw, body.target.yaw,
                                select_speed(angle_difference(body.current.yaw, body.target.yaw), body_speed, g_ai_aim_min_speed, g_ai_aim_min_angle, g_ai_aim_max_angle),
                                time_delta);
    m_object->angle_lerp_bounds(body.current.pitch, body.target.pitch,
                                select_speed(angle_difference(body.current.pitch, body.target.pitch), body_speed, g_ai_aim_min_speed, g_ai_aim_min_angle, g_ai_aim_max_angle),
                                time_delta);

    m_object->angle_lerp_bounds(head.current.yaw, head.target.yaw,
                                select_speed(angle_difference(head.current.yaw, head.target.yaw), head_speed, g_ai_aim_min_speed, g_ai_aim_min_angle, g_ai_aim_max_angle),
                                time_delta);
    m_object->angle_lerp_bounds(head.current.pitch, head.target.pitch,
                                select_speed(angle_difference(head.current.pitch, head.target.pitch), head_speed, g_ai_aim_min_speed, g_ai_aim_min_angle, g_ai_aim_max_angle),
                                time_delta);
    ...
    if (object().cName() == "level_prefix_stalker")
    {
        ...
        Msg("[%6d][%s] AFTER  HEAD [%f][%f] -> [%f][%f]", Device.dwTimeGlobal, object().cName().c_str(), object().movement().m_head.current.yaw,
            object().movement().m_head.current.pitch, object().movement().m_head.target.yaw, object().movement().m_head.target.pitch);
    }
#endif // #ifdef SIGHT_DEBUG
...
Fvector CSightManager::aiming_position() const
{
    ...
    case eSightTypeCurrentDirection: {
        ...
        VERIFY2(result.magnitude() < 100000.f,
                make_string("[%f][%f][%f] [%f][%f] [%f]", VPUSH(object().Position()), -object().movement().m_head.current.yaw, -object().movement().m_head.current.pitch,
                            fake_distance));
    ...
    case eSightTypePathDirection: {
        ...
        VERIFY2(
            ...
            make_string("[%f][%f][%f] [%f][%f] [%f]", VPUSH(object().Position()), -object().movement().m_head.target.yaw, -object().movement().m_head.target.pitch, fake_distance));
        VERIFY(_valid(result));
        break;
    }
...
void CSightManager::process_action(float const time_delta)
{
    ...
    //	if ( object().cName() == "level_prefix_stalker" ) {
    //		Msg							("[%6d][%6d] [%f] + [%f] = [%f] ([%f])",  Device.dwFrame, Device.dwTimeGlobal, m_current.m_head.m_factor,
    // s_factor_lerp_speed*time_delta,		lerp ( m_current.m_head.m_factor,		factors.x, s_factor_lerp_speed*time_delta ), factors.x ); 		Msg
    //("[%6d][%6d] [%f] + [%f] = [%f] ([%f])",  Device.dwFrame, Device.dwTimeGlobal, m_current.m_shoulder.m_factor,	s_factor_lerp_speed*time_delta,		lerp (
    // m_current.m_shoulder.m_factor,	factors.y, s_factor_lerp_speed*time_delta ), factors.y ); 		Msg							("[%6d][%6d] [%f] + [%f] = [%f] ([%f])",
    // Device.dwFrame, Device.dwTimeGlobal, m_current.m_spine.m_factor,	s_factor_lerp_speed*time_delta,		lerp ( m_current.m_spine.m_factor,		factors.z,
    ...
    Fvector const angles = Fvector().set(angle_normalize_signed(-(head.current.pitch - body.current.pitch)), angle_normalize_signed(-(head.current.yaw - body.current.yaw)),
...
void CSightManager::compute_aiming(float const time_delta, float const angular_speed)
{
    ...
    case aiming_weapon: {
        ...
        aimers::weapon aimer(&object(), m_animation_id.c_str(), m_animation_frame == animation_frame_start, aiming_position(),
                             pSettings->r_string(object().cNameSect().c_str(), "bone_spin"), pSettings->r_string(object().cNameSect().c_str(), "bone_shoulder"),
                             pSettings->r_string(object().cNameSect().c_str(), "weapon_bone0"), pSettings->r_string(object().cNameSect().c_str(), "weapon_bone2"),
