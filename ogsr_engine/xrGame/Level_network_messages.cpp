...
static bool SimmulateNetworkLag()
{
    ...
    {
        max_lag_time = Device.dwTimeGlobal + Random.randI(lag_simmulator_min_ping, lag_simmulator_max_ping);
        return false;
    }
    return true;
}
...
void CLevel::ClientReceive()
{
    ...
    for (NET_Packet* P = net_msg_Retreive(); P; P = net_msg_Retreive())
    {
        ...
        case M_SPAWN: {
            ...
            {
                Msg("! Unconventional M_SPAWN received : map_data[%s] | bReady[%s] | deny_m_spawn[%s]", (map_data.m_map_sync_received) ? "true" : "false",
