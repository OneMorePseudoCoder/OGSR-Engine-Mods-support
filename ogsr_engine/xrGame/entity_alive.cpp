...
void CEntityAlive::Hit(SHit* pHDS)
{
    ...
    bool IsBloodmarkNeededForHitType = (HDS.hit_type == ALife::eHitTypeFireWound) || (HDS.hit_type == ALife::eHitTypeWound) || (HDS.hit_type == ALife::eHitTypeWound_2);

    if (IsBloodmarkNeededForHitType && !use_simplified_visual())
    {
...
{
    inline bool operator()(CCF_Skeleton::SElement const& element, u16 element_id) const { return element.elem_id < element_id; }
}; // struct element_predicate

struct sort_surface_area_predicate
{
    inline bool operator()(std::pair<u16, float> const& left, std::pair<u16, float> const& right) const { return left.second > right.second; }
}; // struct sort_surface_area_predicate
...
#pragma todo("SIMP: см комментарии ниже")
BOOL g_ai_use_old_vision = 1; //0;
...
Fvector CEntityAlive::get_new_local_point_on_mesh(u16& bone_id) const
{
    ...
    for (; i != e; ++i)
    {
#pragma todo("SIMP: тут чето вылетает по невалидным костям на загрузке сейвов (и не только), мб под многопоточный feel vision не хватает локов каких то или хз. Надо разбираться. Если в консоли сделать ai_use_old_vision 1 то этот код не юзается и вылетов нет")
    ...
    float const selected_area = Random.randF(hit_bones_surface_area);
