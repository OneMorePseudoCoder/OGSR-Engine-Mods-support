...
bool CControlAnimationBase::accel_chain_test()
{
    ...
    for (VELOCITY_CHAIN_VEC_IT I = m_accel.chain.begin(); I != m_accel.chain.end(); I++)
    {
        ...
        for (SEQ_VECTOR_IT IT = I->begin() + 1; IT != I->end(); IT++)
        {
            ...
            xr_sprintf(error_msg, "Incompatible speed ranges. Monster[%s] From animation  [%s] To animation [%s]", *m_object->cName(), *anim_from->target_name,
                       *anim_to->target_name);
            VERIFY2((anim_to->velocity.velocity.linear * anim_to->velocity.min_factor) < (anim_from->velocity.velocity.linear * anim_from->velocity.max_factor), error_msg);
