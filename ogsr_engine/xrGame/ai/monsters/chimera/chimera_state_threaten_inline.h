...
{
    if (this->object->tfGetRelationType(this->object->EnemyMan.get_enemy()) == ALife::eRelationTypeWorstEnemy)
        return false;
    if (this->object->Position().distance_to(this->object->EnemyMan.get_enemy_position()) < MIN_DIST_TO_ENEMY)
        return false;
    if (this->object->HitMemory.is_hit())
        return false;
    if (this->object->hear_dangerous_sound)
        return false;
    if (this->m_last_time_threaten + THREATEN_DELAY > Device.dwTimeGlobal)
        return false;
