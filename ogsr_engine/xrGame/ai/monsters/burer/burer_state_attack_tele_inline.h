...
void CStateBurerAttackTele<Object>::FindFreeObjects(xr_vector<CObject*>& tpObjects, const Fvector& pos)
{
    ...
    for (u32 i = 0; i < tpObjects.size(); i++)
    {
        ...
        if (grenade || // grenades are handled by HandleGrenades function
            ...
            (obj->m_pPhysicsShell->getMass() < this->object->m_tele_object_min_mass) || (obj->m_pPhysicsShell->getMass() > this->object->m_tele_object_max_mass) ||
            (obj == this->object) || this->object->CTelekinesis::is_active_object(obj) || !obj->m_pPhysicsShell->get_ApplyByGravity() ||
...
void CStateBurerAttackTele<Object>::HandleGrenades()
{
    ...
    for (u32 i = 0; i < m_nearest.size(); ++i)
    {
        ...
        if (!grenade || !grenade->PPhysicsShell() || !grenade->PPhysicsShell()->isActive() || this->object->CTelekinesis::is_active_object(grenade) ||
