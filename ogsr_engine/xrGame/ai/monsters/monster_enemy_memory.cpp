...
void CMonsterEnemyMemory::update()
{
    ...
    {
        const CEntityAlive* ea_enemy = enemy;
        const bool feel_enemy = monster->Position().distance_to(ea_enemy->Position()) < monster->get_feel_enemy_max_distance();

        if (feel_enemy || monster->memory().visual().visible_now(ea_enemy))
            add_enemy(ea_enemy);
    }
