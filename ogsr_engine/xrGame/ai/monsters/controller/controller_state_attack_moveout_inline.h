...
bool CStateControlMoveOutAbstract::check_completion()
{
    ...
    if (this->time_state_started + 10000 < time())
        return true;
    if (this->object->HitMemory.is_hit() && this->object->HitMemory.get_last_hit_time() > this->time_state_started)
        return true;

    if (this->object->EnemyMan.get_enemy_position().distance_to(this->object->Position()) < 1.f)
        return true;
