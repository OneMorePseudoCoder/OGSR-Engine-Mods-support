...
void ATTACK_ON_RUN_STATE::update_movement_target()
{
    ...
    {
        if (angle_between_vectors(this->object->Direction(), self2enemy) > deg2rad(140.f) && self2predicted_mag < 4.f && current_time() > m_phaze_chosen_time + 3000)
        {
...
void ATTACK_ON_RUN_STATE::update_attack()
{
    ...
    else if (!m_is_jumping && !this->object->anim().has_override_animation() && m_phaze == go_close)
    {
        ...
        if (can_attack)
        {
            ...
            bool const got_animation_info = this->object->anim().get_animation_info(anim, m_animation_index[m_attack_side], motion, attack_animation_length);
