...
bool CControlAnimationBase::AddAnim(EMotionAnim ma, pcstr tn, int s_id, SVelocityParam* vel, EPState p_s, const SAnimItem::Effects& fxs, bool required /*= true*/)
{
    ...
    {
        string256 buf;
        xr_strconcat(buf, tn, "0");
        if (!visual->ID_Cycle_Safe(buf).valid() && !visual->ID_Cycle_Safe(tn).valid())
        {
            return false;
        }
    }
