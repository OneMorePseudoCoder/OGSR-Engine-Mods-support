...
{
    bool const in_camp_state = this->prev_substate != u32(-1) && this->prev_substate != eStateAttack_HomePoint_Hide;
    ...
    if (this->object->ai_location().level_vertex_id() == m_target_node && !this->object->control().path_builder().is_moving_on_path())
    {
        return true;
    }
