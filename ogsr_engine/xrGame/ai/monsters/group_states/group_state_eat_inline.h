...
void CStateGroupEatAbstract::setup_substates()
{
    ...
    if (this->current_substate == eStateEat_CorpseApproachRun)
    {
        ...
        if ((corpse_->m_pPhysicsShell == NULL) || (!corpse_->m_pPhysicsShell->isActive()))
        {
            ...
            nearest_bone_pos = this->object->character_physics_support()->movement()->PHCaptureGetNearestElemPos(corpse_);
...
bool CStateGroupEatAbstract::check_start_conditions()
{
    if (this->object->EatedCorpse)
        return true;
    return (this->object->CorpseMan.get_corpse() && this->object->Home->at_home(this->object->CorpseMan.get_corpse()->Position()) && hungry() &&
