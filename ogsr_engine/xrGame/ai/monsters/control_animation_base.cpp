...
// DEBUG purpose only
const char* dbg_action_name_table[] = {"ACT_STAND_IDLE", "ACT_SIT_IDLE", "ACT_LIE_IDLE", "ACT_WALK_FWD", "ACT_WALK_BKWD", "ACT_RUN",         "ACT_EAT",
...
bool CControlAnimationBase::get_animation_info(EMotionAnim anim, u32 index, MotionID& motion, float& length) const
{
    ...
    string1024 animation_name_buffer = {};
...
void CControlAnimationBase::UpdateAnimCount()
{
    ...
    for (size_t idx : subjectsToDelete)
    {
        ...
        while (true)
        {
            auto it = std::find_if(m_tReplacedAnims.begin(), m_tReplacedAnims.end(), [idx](const SReplacedAnim& ranim) { return ranim.cur_anim == idx || ranim.new_anim == idx; });
            if (it == m_tReplacedAnims.end())
                break;
            m_tReplacedAnims.erase(it);
        }
        const auto it = std::find_if(m_tMotions.begin(), m_tMotions.end(), [idx](const std::pair<EAction, SMotionItem>& item) { return item.second.anim == idx; });
