...
{
    const bool health_step_lost = this->object->conditions().GetHealth() < m_last_health - detail::bloodsucker::loose_health_diff;
...
void CStateMonsterBackstubEnemy<_Object>::execute()
{
    ...
    if (this->object->conditions().GetHealth() < m_last_health - detail::bloodsucker::loose_health_diff && Device.dwTimeGlobal > m_next_change_behaviour_tick)
    {
...
bool CStateMonsterBackstubEnemy<_Object>::check_completion()
{
    ...
    const bool real_path_end = fis_zero(data.completion_dist) ? (data.point.distance_to_xz(this->object->Position()) < ai().level_graph().header().cell_size()) : true;
