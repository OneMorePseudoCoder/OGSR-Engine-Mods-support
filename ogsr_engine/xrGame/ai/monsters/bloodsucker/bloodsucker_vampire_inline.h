...
bool CStateBloodsuckerVampireAbstract::check_start_conditions()
{
    ...
    if (CAI_Bloodsucker::m_time_last_vampire + this->object->m_vampire_min_delay > Device.dwTimeGlobal)
        return false;
...
{
    // если убежал
    if ((this->current_substate == eStateVampire_Hide) && this->get_state_current()->check_completion())
        return true;
    ...
    if ((this->current_substate != eStateVampire_Execute) && this->object->CControlledActor::is_controlling())
        return true;
