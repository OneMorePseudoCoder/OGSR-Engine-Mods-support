...
CBaseMonster::SDebugInfo CBaseMonster::show_debug_info()
{
    ...
    xr_sprintf(text, "MinDist[%.3f] MaxDist[%.3f] As_Step[%.3f] As_MinDist[%.3f]", MeleeChecker.get_min_distance(), MeleeChecker.get_max_distance(), MeleeChecker.dbg_as_step(),
...
void CBaseMonster::debug_fsm()
{
    ...
    {
        xr_sprintf(st, "VELOCITY [%f,%f,%f] Value[%f]", VPUSH(entity->character_physics_support()->movement()->GetVelocity()),
