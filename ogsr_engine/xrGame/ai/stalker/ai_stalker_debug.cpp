...
void CAI_Stalker::debug_text()
{
    ...
    if (memory().danger().selected() && memory().danger().selected()->object())
    {
        ...
        DBG_OutText("%s%s%stime      : %.3f (%.3f)", indent, indent, indent, float(memory().danger().selected()->time()) / 1000.f,
    ...
    if (g_Alive())
    {
        ...
        if (g_Alive())
        {
            ...
            {
                DBG_OutText("%scover         : [%s][%f][%f][%f]", indent, agent_manager().member().member(this).cover()->m_is_smart_cover ? "smart" : "generated",
...
{
    inherited::OnHUDDraw(hud, context_id, root);
    m_dbg_hud_draw = true;
}
