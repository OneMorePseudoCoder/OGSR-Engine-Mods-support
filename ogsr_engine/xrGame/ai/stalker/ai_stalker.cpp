...
void CAI_Stalker::LoadSounds(LPCSTR section)
{
    ...
    {
        sound().add(pSettings->r_string(section, "sound_wounded"), 100, SOUND_TYPE_MONSTER_TALKING, 4, u32(eStalkerSoundMaskWounded), eStalkerSoundWounded, head_bone_name,
                    xr_new<CStalkerSoundData>(this));
    }
    else
    {
        sound().add(pSettings->r_string(section, "sound_tolls"), 100, SOUND_TYPE_MONSTER_TALKING, 4, u32(eStalkerSoundMaskWounded), eStalkerSoundWounded, head_bone_name,
                    xr_new<CStalkerSoundData>(this));
    }
    ...
    {
        sound().add(pSettings->r_string(section, "sound_enemy_lost_no_allies"), 100, SOUND_TYPE_MONSTER_TALKING, 5, u32(eStalkerSoundMaskEnemyLostNoAllies),
                    eStalkerSoundEnemyLostNoAllies, head_bone_name, xr_new<CStalkerSoundData>(this));
    }
    else
    {
        sound().add(pSettings->r_string(section, "sound_search1_no_allies"), 100, SOUND_TYPE_MONSTER_TALKING, 5, u32(eStalkerSoundMaskEnemyLostNoAllies),
                    eStalkerSoundEnemyLostNoAllies, head_bone_name, xr_new<CStalkerSoundData>(this));
    }
    if (pSettings->line_exist(section, "sound_enemy_lost_with_allies"))
    {
        sound().add(pSettings->r_string(section, "sound_enemy_lost_with_allies"), 100, SOUND_TYPE_MONSTER_TALKING, 5, u32(eStalkerSoundMaskEnemyLostWithAllies),
                    eStalkerSoundEnemyLostWithAllies, head_bone_name, xr_new<CStalkerSoundData>(this));
    }
    else
    {
        sound().add(pSettings->r_string(section, "sound_search1_with_allies"), 100, SOUND_TYPE_MONSTER_TALKING, 5, u32(eStalkerSoundMaskEnemyLostWithAllies),
                    eStalkerSoundEnemyLostWithAllies, head_bone_name, xr_new<CStalkerSoundData>(this));
    }
    ...
    // u32(eStalkerSoundMaskMovingInDanger),			eStalkerSoundWalkingInDanger,			head_bone_name, xr_new<CStalkerSoundData>(this));
    ...
    sound().add(pSettings->r_string(section, "sound_throw_grenade"), 100, SOUND_TYPE_MONSTER_TALKING, 5, u32(eStalkerSoundMaskKillWounded), eStalkerSoundThrowGrenade,
                head_bone_name, xr_new<CStalkerSoundData>(this));
}
...
void CAI_Stalker::reload(LPCSTR section)
{
    ...
    {
        const u32 min_queue_size_far = READ_IF_EXISTS(pSettings, r_s32, queue_sect, "weapon_min_queue_size_far", 1);
        const u32 max_queue_size_far = READ_IF_EXISTS(pSettings, r_s32, queue_sect, "weapon_max_queue_size_far", 6);
        const u32 min_queue_interval_far = READ_IF_EXISTS(pSettings, r_s32, queue_sect, "weapon_min_queue_interval_far", 500);
        const u32 max_queue_interval_far = READ_IF_EXISTS(pSettings, r_s32, queue_sect, "weapon_max_queue_interval_far", 1000);

        const u32 min_queue_size_medium = READ_IF_EXISTS(pSettings, r_s32, queue_sect, "weapon_min_queue_size_medium", 4);
        const u32 max_queue_size_medium = READ_IF_EXISTS(pSettings, r_s32, queue_sect, "weapon_max_queue_size_medium", 6);
        const u32 min_queue_interval_medium = READ_IF_EXISTS(pSettings, r_s32, queue_sect, "weapon_min_queue_interval_medium", 500);
        const u32 max_queue_interval_medium = READ_IF_EXISTS(pSettings, r_s32, queue_sect, "weapon_max_queue_interval_medium", 750);

        const u32 min_queue_size_close = READ_IF_EXISTS(pSettings, r_s32, queue_sect, "weapon_min_queue_size_close", 4);
        const u32 max_queue_size_close = READ_IF_EXISTS(pSettings, r_s32, queue_sect, "weapon_max_queue_size_close", 10);
        const u32 min_queue_interval_close = READ_IF_EXISTS(pSettings, r_s32, queue_sect, "weapon_min_queue_interval_close", 300);
        const u32 max_queue_interval_close = READ_IF_EXISTS(pSettings, r_s32, queue_sect, "weapon_max_queue_interval_close", 500);
...
void CAI_Stalker::create_anim_mov_ctrl(CBlend* b, Fmatrix* start_pose, bool local_animation) { inherited::create_anim_mov_ctrl(b, start_pose, local_animation); }
...
CMovementManager* CAI_Stalker::create_movement_manager() { return (m_movement_manager = xr_new<stalker_movement_manager_smart_cover>(this)); }
...
void CAI_Stalker::fill_bones_body_parts(LPCSTR bone_id, const ECriticalWoundType& wound_type)
{
    ...
    const auto& body_part_section = pSettings->r_section(body_part_section_id);
    for (const auto& name : body_part_section.Ordered_Data | std::views::keys)
        m_bones_body_parts.insert(std::make_pair(kinematics->LL_BoneID(name), u32(wound_type)));
}
