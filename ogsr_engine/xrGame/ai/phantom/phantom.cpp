...
BOOL CPhantom::net_Spawn(CSE_Abstract* DC)
{
    ...
    // [bug] Баг в CPhantom::net_Spawn — иногда на загрузке игры звезды могут сойтись так,
    // что в m_enemy попадёт null, что тут же приведёт к вылету.
    //
    // В качестве исправления подставляем указатель на самого фантома — увы,
    // в xform заведутся nan'ы при попытке установки ориентации,
    // так что сразу и в стейт пропишем терминальный.
    if (!m_enemy)
    {
        m_enemy = this;
        m_TgtState = stShoot;
