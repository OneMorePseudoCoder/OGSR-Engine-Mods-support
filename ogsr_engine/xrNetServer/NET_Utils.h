...
public:
    ...
    NET_Packet() : B(), r_pos(0), timeReceive(0) {}

    NET_Packet(const void* data, const size_t size) : NET_Packet() { set(data, size); }
    ...
    // writing - utilities
    void w_float(const float a) { w(&a, sizeof(float)); } // float

    void w_vec3(const Fvector& a) { w(&a, 3 * sizeof(float)); } // vec3

    void w_vec4(const Fvector4& a) { w(&a, 4 * sizeof(float)); } // vec4

    void w_u64(const u64 a) { w(&a, sizeof(u64)); } // qword (8b)

    void w_s64(const s64 a) { w(&a, sizeof(s64)); } // qword (8b)

    void w_u32(const u32 a) { w(&a, sizeof(u32)); } // dword (4b)

    void w_s32(const s32 a) { w(&a, sizeof(s32)); } // dword (4b)

    void w_u16(const u16 a) { w(&a, sizeof(u16)); } // word (2b)

    void w_s16(const s16 a) { w(&a, sizeof(s16)); } // word (2b)

    void w_u8(const u8 a) { w(&a, sizeof(u8)); } // byte (1b)

    void w_s8(const s8 a) { w(&a, sizeof(s8)); } // byte (1b)
    ...
    void w_stringZ(const char* S) { w(S, xr_strlen(S) + 1); }
    ...
    // reading
    void read_start() { r_pos = 0; }

    u32 r_begin(u16& type);
    void r_seek(const u32 pos);
    u32 r_tell() const { return r_pos; }
    ...
    bool r_eof() const { return r_pos >= B.count; }

    u32 r_elapsed() const { return B.count - r_pos; }

    void r_advance(const u32 size);

    // reading - utilities
    void r_vec3(Fvector& A) { r(&A, sizeof(Fvector)); }

    void r_vec4(Fvector4& A) { r(&A, sizeof(Fvector4)); }

    void r_float(float& A) { r(&A, sizeof(float)); }

    void r_u64(u64& A) { r(&A, sizeof(u64)); }

    void r_s64(s64& A) { r(&A, sizeof(s64)); }

    void r_u32(u32& A) { r(&A, sizeof(u32)); }

    void r_s32(s32& A) { r(&A, sizeof(s32)); }

    void r_u16(u16& A) { r(&A, sizeof(u16)); }

    void r_s16(s16& A) { r(&A, sizeof(s16)); }

    void r_u8(u8& A) { r(&A, sizeof(u8)); }

    void r_s8(s8& A) { r(&A, sizeof(s8)); }
