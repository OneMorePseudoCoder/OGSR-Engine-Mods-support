...
struct CCloner
{
    ...
    IC static void clone(LPCSTR source, LPCSTR& destination) { destination = source; }

    IC static void clone(LPSTR source, LPSTR& destination) { destination = xr_strdup(source); }

    IC static void clone(const shared_str& source, shared_str& destination) { destination = source; }
