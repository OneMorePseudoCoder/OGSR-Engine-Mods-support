...
bool GetLShift() { return !!pInput->iGetAsyncKeyState(DIK_LSHIFT); }
bool GetRShift() { return !!pInput->iGetAsyncKeyState(DIK_RSHIFT); }
...
bool GetControl() { return !!pInput->iGetAsyncKeyState(DIK_LCONTROL) || !!pInput->iGetAsyncKeyState(DIK_RCONTROL); }
bool GetLControl() { return !!pInput->iGetAsyncKeyState(DIK_LCONTROL); }
bool GetRControl() { return !!pInput->iGetAsyncKeyState(DIK_RCONTROL); }
...
void CScriptEngine::script_register(lua_State* L)
{
    module(
        L)[def("log1", (void (*)(LPCSTR))&Log), 
           ...
           def("time_global_async", [] { return Device.TimerAsync_MMT(); }),

           def("GetShift", &GetShift), 
           def("GetLShift", &GetLShift), 
           def("GetRShift", &GetRShift), 

           def("GetLAlt", &GetLAlt), 
           def("GetRAlt", &GetRAlt), 
           def("GetAlt", &GetAlt),

           def("GetLControl", &GetLControl), 
           def("GetRControl", &GetRControl), 
           def("GetControl", &GetControl),

           def("device", [] { return &Device; }),
           def("__debugbreak", [] { __debugbreak(); })];
}