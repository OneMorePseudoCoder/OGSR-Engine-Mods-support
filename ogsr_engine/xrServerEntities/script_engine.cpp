...
void CScriptEngine::LogTable(lua_State* l, LPCSTR S, int level)
{
    ...
    while (lua_next(l, -2) != 0)
    {
...

        {
            ...
            switch (kt)
            {
            case LUA_TBOOLEAN: tk = lua_toboolean(l, -2) ? "true" : "false"; break;
            case LUA_TNUMBER: tk = _itoa(lua_tointeger(l, -2), tmp, 10); break;
            case LUA_TSTRING: tk = lua_tostring(l, -2); break;
            default: tk = _itoa(cnt, tmp, 10); break;
            }
