...
void setup_location_types_section(GameGraph::TERRAIN_VECTOR& m_vertex_types, CInifile const* ini, LPCSTR section)
{
    ...
    const auto& sect = ini->r_section(section);
    for (const auto& S : sect.Ordered_Data | std::views::keys)
    {
        string16 I;
        u32 N = _GetItemCount(S.c_str());

        if (N != GameGraph::LOCATION_TYPE_COUNT)
            continue;

        for (u32 j = 0; j < GameGraph::LOCATION_TYPE_COUNT; ++j)
            terrain_mask.tMask[j] = GameGraph::_LOCATION_ID(atoi(_GetItem(S.c_str(), j, I)));
...
bool CSE_ALifeCreatureAbstract::can_switch_offline() const { return (inherited::can_switch_offline() && (get_health() > 0.f)); }
...
bool CSE_ALifeMonsterAbstract::need_update(CSE_ALifeDynamicObject* object) { return (CSE_ALifeSchedulable::need_update(object) && (get_health() > EPS_L)); }
